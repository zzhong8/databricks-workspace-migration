...
df_sql_query_asda_nestlea = """
    select SALES_DT, BASELINE_POS_ITEM_QTY from asda_nestlecore_uk_retail_alert_im.drfe_forecast_baseline_unit
    where SALES_DT >= '2022-07-01'
    order by SALES_DT
"""

data_vault_data1a = spark.sql(df_sql_query_asda_nestlea)

total_baseline_sales_by_date1a = data_vault_data1a.select("SALES_DT", "BASELINE_POS_ITEM_QTY").orderBy("SALES_DT").groupBy("SALES_DT").sum()

display(total_baseline_sales_by_date1a)
