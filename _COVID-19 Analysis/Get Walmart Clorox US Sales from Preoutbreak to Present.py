...
# MAGIC %sql
# MAGIC
# MAGIC desc dw_walmart_clorox_us_dv.vw_latest_sat_epos

# COMMAND ----------

# MAGIC %sql
# MAGIC
# MAGIC desc dw_walmart_clorox_us_dv.dvs_epos

# COMMAND ----------

# MAGIC %sql
# MAGIC
# MAGIC select * from dw_walmart_clorox_us_dv.dvs_epos
...
# MAGIC %sql
# MAGIC
# MAGIC select * from dw_walmart_clorox_us_dv.hub_item

# COMMAND ----------

# MAGIC %sql
# MAGIC
# MAGIC select * from dw_walmart_clorox_us_dv.hub_ORGANIZATION_UNIT

# COMMAND ----------

# MAGIC %sql
# MAGIC
# MAGIC select * from dw_walmart_clorox_us_dv.link_epos
...
hi.ITEM_NBR,
    ...
    count(distinct se.SALES_DT) as num_sales_days,
    min(se.SALES_DT) as min_sales_date,
    max(se.SALES_DT) as max_sales_date,
    
    sum(se.POS_QTY) as total_sales_units,
    sum(se.POS_SALES) as total_sales_dollars,
    
    avg(se.POS_QTY) as avg_sales_units,
    avg(se.POS_SALES) as avg_sales_dollars

from
dw_walmart_clorox_us_dv.sat_epos se
join
dw_walmart_clorox_us_dv.link_epos le
on
se.LINK_EPOS_HK = se.LINK_EPOS_HK
join
dw_walmart_clorox_us_dv.hub_item hi
on
le.HUB_ITEM_HK = hi.HUB_ITEM_HK
join
dw_walmart_clorox_us_dv.hub_ORGANIZATION_UNIT hou
on
le.HUB_ORGANIZATION_UNIT_HK = hou.HUB_ORGANIZATION_UNIT_HK
where se.SALES_DT >= '2020-01-01'
and se.POS_SALES != 0
...
data_vault_data = spark.sql(sql_read_pos_data)

# COMMAND ----------

# df_agg = data_vault_data.groupBy('STORE_NBR', 'ITEM_NBR').agg(pyf.count("*").alias('NUM_DAYS_WITH_SALES'), pyf.sum("POS_SALES").alias('TOTAL_POS_SALES'), pyf.avg("POS_SALES").alias('AVG_POS_SALES')).orderBy(["STORE_NBR", "ITEM_NBR"], ascending=True)

# COMMAND ----------

### Save the final data set ###
data_vault_data.coalesce(1).write.format('csv')\
    .mode('overwrite')\
    .option('overwriteSchema', True)\
    .option('header', True)\
    .save(f'/mnt/processed/hugh/walmart-clorox-analysis-20220906/')

# COMMAND ----------

# MAGIC %sql
# MAGIC
# MAGIC select count(distinct LINK_EPOS_HK) from dw_walmart_clorox_us_dv.sat_epos
# MAGIC where SALES_DT >= '2020-01-01'
...
# MAGIC %sql
# MAGIC
# MAGIC select * from dw_walmart_clorox_us_dv.sat_epos
# MAGIC where SALES_DT >= '2020-01-01'
...
# MAGIC %sql
# MAGIC
# MAGIC select distinct sales_dt from dw_walmart_clorox_us_dv.sat_epos
# MAGIC where SALES_DT >= '2020-01-01'
...
data_vault_data1 = data_vault_data1.where((pyf.col("SALES_DT") < pyf.lit("2020-08-01")) & (pyf.col("SALES_DT") >= pyf.lit("2020-02-23")))
...
input_data_list =  [('walmart', 'clorox', 'us')]
