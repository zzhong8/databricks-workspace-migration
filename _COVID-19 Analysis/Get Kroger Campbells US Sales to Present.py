...
# MAGIC %sql
# MAGIC
# MAGIC select * from retail_alert_kroger_campbells_us_im.DRFE_FORECAST_BASELINE_UNIT

# COMMAND ----------

# Read POS data
data_vault_data1 = read_pos_data(SOURCE_SYSTEM, RETAILER, CLIENT, COUNTRY_CODE, sqlContext).repartition('SALES_DT')
data_vault_data1 = data_vault_data1.where((pyf.col("SALES_DT") < pyf.lit("2025-01-31")) & (pyf.col("SALES_DT") >= pyf.lit("2024-01-01")))
...
# MAGIC %sql
# MAGIC
# MAGIC select max(SALES_DT) from retail_alert_kroger_campbells_us_im.DRFE_FORECAST_BASELINE_UNIT

# COMMAND ----------

# MAGIC %sql
# MAGIC
# MAGIC select max(SALES_DT) from retail_alert_kroger_campbells_us_im.alert_on_shelf_availability

# COMMAND ----------

# MAGIC %sql
# MAGIC
# MAGIC select SALES_DT, count(*) from retail_alert_kroger_campbells_us_im.alert_on_shelf_availability
...
# MAGIC %sql
# MAGIC
# MAGIC select SALES_DT, count(*) from retail_alert_kroger_campbells_us_im.vw_latest_alert_on_shelf_availability
# MAGIC group by SALES_DT
# MAGIC order by SALES_DT
...
data_vault_data_zero_sales = data_vault_data1.where(pyf.col('POS_ITEM_QTY') == 0)
...
data_vault_data_zero_sales.count()
