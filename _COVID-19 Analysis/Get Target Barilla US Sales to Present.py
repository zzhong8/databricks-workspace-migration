...
# MAGIC %sql
# MAGIC
# MAGIC select max(SALES_DT) from retail_alert_target_barilla_us_im.DRFE_FORECAST_BASELINE_UNIT

# COMMAND ----------

# MAGIC %sql
# MAGIC
# MAGIC select SALES_DT, count(*) from retail_alert_target_barilla_us_im.alert_on_shelf_availability
...
# MAGIC %sql
# MAGIC
# MAGIC select SALES_DT, count(*) from bigred_target_barilla_us_dv.vw_latest_sat_epos_summary
# MAGIC group by SALES_DT
# MAGIC order by SALES_DT
...
# MAGIC %sql
# MAGIC
# MAGIC select SALES_DT, count(*) from retail_alert_target_barilla_us_im.vw_latest_alert_on_shelf_availability
# MAGIC group by SALES_DT
# MAGIC order by SALES_DT
...
data_vault_data1 = data_vault_data1.where((pyf.col("SALES_DT") < pyf.lit("2024-01-01")) & (pyf.col("SALES_DT") >= pyf.lit("2020-01-01")))
