...
data_vault_data0 = data_vault_data.where((pyf.col("SALES_DT") < pyf.lit("2020-09-01")) & (pyf.col("SALES_DT") >= pyf.lit("2018-01-01")))

total_sales_by_date0 = data_vault_data0.select("SALES_DT", "POS_ITEM_QTY").orderBy("SALES_DT").groupBy("SALES_DT").sum()

display(total_sales_by_date0)

# COMMAND ----------

total_inventory_by_date0 = data_vault_data0.select("SALES_DT", "ON_HAND_INVENTORY_QTY").orderBy("SALES_DT").groupBy("SALES_DT").sum()

display(total_inventory_by_date0)

# COMMAND ----------

# Read POS data
data_vault_data1 = data_vault_data.where((pyf.col("SALES_DT") < pyf.lit("2020-09-01")) & (pyf.col("SALES_DT") >= pyf.lit("2020-01-01")))
...
total_sales_by_item = data_vault_data1.select("RETAILER_ITEM_ID", "POS_ITEM_QTY").groupBy("RETAILER_ITEM_ID")\
   .agg(pyf.sum('POS_ITEM_QTY').alias('TOTAL_POS_ITEM_QTY'))

total_sales_by_item_ordered = total_sales_by_item.orderBy("TOTAL_POS_ITEM_QTY", ascending=False)

display(total_sales_by_item_ordered)
...
input_data_list =  [('tesco', 'generalmills', 'uk')]
...
# result_filtered_1 = result.filter(result.RETAILER_ITEM_ID == 50515111)
# result_filtered_2 = result.filter(result.RETAILER_ITEM_ID == 50515115)
# result_filtered_3 = result.filter(result.RETAILER_ITEM_ID == 50515114)
# result_filtered_4 = result.filter(result.RETAILER_ITEM_ID == 50226495)
# result_filtered_5 = result.filter(result.RETAILER_ITEM_ID == 50515110)
# result_filtered_6 = result.filter(result.RETAILER_ITEM_ID == 50159248)
# result_filtered_7 = result.filter(result.RETAILER_ITEM_ID == 6378250)
# result_filtered_8 = result.filter(result.RETAILER_ITEM_ID == 5713513)
# result_filtered_9 = result.filter(result.RETAILER_ITEM_ID == 9071290)
# result_filtered_10 = result.filter(result.RETAILER_ITEM_ID == 9085150)
