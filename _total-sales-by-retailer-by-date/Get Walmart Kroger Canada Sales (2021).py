...
# Read POS data
data_vault_data_kroger_barilla = read_pos_data(SOURCE_SYSTEM, RETAILER, CLIENT, COUNTRY_CODE, sqlContext).repartition('SALES_DT')
data_vault_data_kroger_barilla = data_vault_data_kroger_barilla.where((pyf.col("SALES_DT") < pyf.lit("2021-06-01")) & (pyf.col("SALES_DT") >= pyf.lit("2021-02-01")))

# COMMAND ----------

total_sales_by_date_kroger_barilla = data_vault_data_kroger_barilla.select("SALES_DT", "POS_ITEM_QTY").orderBy("SALES_DT").groupBy("SALES_DT").sum()
display(total_sales_by_date_kroger_barilla)

# COMMAND ----------

total_inventory_by_date_kroger_barilla = data_vault_data_kroger_barilla.select("SALES_DT", "ON_HAND_INVENTORY_QTY").orderBy("SALES_DT").groupBy("SALES_DT").sum()
display(total_inventory_by_date_kroger_barilla)
