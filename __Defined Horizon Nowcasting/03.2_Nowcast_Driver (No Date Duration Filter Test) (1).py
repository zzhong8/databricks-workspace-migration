...
data_over_threshold = loaded_data
...
data_over_threshold_predictor_cols = data_over_threshold.select(*predictor_cols)

display(data_over_threshold_predictor_cols.select([pyf.count(pyf.when(pyf.isnan(c) | pyf.col(c).isNull(), c)).alias(c) for c in data_over_threshold_predictor_cols.columns]))
...
data_over_threshold.cache().count()

# COMMAND ----------

data_over_threshold.show(5)

# COMMAND ----------

data_with_final_features.cache().count()
