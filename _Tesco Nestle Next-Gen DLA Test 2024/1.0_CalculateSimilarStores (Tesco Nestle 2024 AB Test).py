...
#TODO: read this information from a table
# Import approved product list for this client
approved_product_list_reference_path = f'/mnt/processed/alerting/approved_product_list/{retailer}_{country_code}_{client}_{company_id}_approved_product_list.csv/'
...
if check_path_exists(approved_product_list_reference_path, 'csv', 'ignore'):
    ...
    df_pos = df_pos.join(
        ...
        df_pos['RETAILER_ITEM_ID'] == approved_product_list['Item Number']
    ).drop(
        'RetailProductCode'
    )
    print(f'{df_pos.cache().count():,}')
...
experiment_name = f'/Users/nafiseh.asghari@mosaic.com/NextGenDLA/Alerting_gen2/{retailer}_{client}_{country_code}_NNModel_Weekly'
...
covered_stores_path = f'/mnt/processed/alerting/fieldTest/covered_stores/{retailer}_{client}_{country_code}_{company_id}_covered_stores_2024.csv/'
...
if check_path_exists(covered_stores_path, 'csv', 'ignore'):
    ...
    if retailer == 'tesco' and client == 'nestlecore':
        df_similar_stores = df_similar_stores.join(
            df_valid_stores,
            df_similar_stores['TEST_ORGANIZATION_UNIT_NUM'] == df_valid_stores['StoreNo'],
            how='left_semi'
        )
        print(f'N = {df_similar_stores.cache().count():,}')
        removed_rows = np.round(((main_size_similar_store - df_similar_stores.count())/ main_size_similar_store) * 100, 2)
