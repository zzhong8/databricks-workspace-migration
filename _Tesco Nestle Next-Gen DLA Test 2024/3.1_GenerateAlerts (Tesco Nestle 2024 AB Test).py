...
# MAGIC %run "./3.0_PreprocessingModules (Tesco Nestle 2024 AB Test)"

# COMMAND ----------

# MAGIC %run "./3.2_PostprocessingModules (Tesco Nestle 2024 AB Test)"
...
if run_similar_store_notebook:
    dbutils.notebook.run(
        '1.0_CalculateSimilarStores (Tesco Nestle 2024 AB Test)', 3600, {
...
if max_date < (today_date - datetime.timedelta(days=1)):
    raise ValueError(
...
df_prepared = df_prepared\
    ...
    .filter(pyf.col('Date') >= pyf.date_sub(pyf.from_utc_timestamp(pyf.current_timestamp(), time_zone), 1))
...
df_prepared = df_prepared.withColumn(
    'LostSalesValue',
    pyf.round(df_prepared['LAST_LOST_SALES'] * df_prepared['PRICE'], 2)
)
print(f'N = {df_prepared.cache().count():,}')
