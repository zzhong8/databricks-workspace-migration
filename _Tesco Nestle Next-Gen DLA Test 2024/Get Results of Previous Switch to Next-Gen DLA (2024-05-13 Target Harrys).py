...
# MAGIC %sql
# MAGIC
# MAGIC desc team_retail_alert_target_syn_us_im.alert_on_shelf_availability
...
# MAGIC %sql
# MAGIC
# MAGIC SELECT SALES_DT, count(*) FROM team_retail_alert_target_syn_us_im.alert_on_shelf_availability
# MAGIC where
# MAGIC Retail_Client = 'target_wildcat_us'
...
# MAGIC %sql
# MAGIC
# MAGIC SELECT SALES_DT, count(DISTINCT RETAILER_ITEM_ID) as num_items_covered_by_osa_alerts FROM team_retail_alert_target_syn_us_im.alert_on_shelf_availability
# MAGIC where
# MAGIC Retail_Client = 'target_wildcat_us'
# MAGIC group by SALES_DT
# MAGIC order by SALES_DT
...
# MAGIC %sql
# MAGIC
# MAGIC SELECT SALES_DT, count(*) as num_osa_alerts FROM team_retail_alert_wildcat_kroger_syn_us_im.alert_on_shelf_availability
# MAGIC -- where
# MAGIC -- Retail_Client = 'kroger_wildcat_us'
# MAGIC group by SALES_DT
# MAGIC order by SALES_DT
...
# MAGIC %sql
# MAGIC
# MAGIC SELECT SALES_DT, count(DISTINCT RETAILER_ITEM_ID) as num_items_covered_by_osa_alerts FROM team_retail_alert_wildcat_kroger_syn_us_im.alert_on_shelf_availability
# MAGIC -- where
# MAGIC -- Retail_Client = 'kroger_wildcat_us'
# MAGIC group by SALES_DT
# MAGIC order by SALES_DT
...
# MAGIC %sql
# MAGIC
# MAGIC SELECT call_date, count(*) as num_problems_detected, sum(total_impact) as total_uplift, count(DISTINCT epos_retailer_item_id) as num_covered_items FROM acosta_retail_analytics_im.ds_intervention_summary
# MAGIC where
# MAGIC mdm_country_id = 1 -- US
# MAGIC and
# MAGIC mdm_client_id = 16540 -- Harrys Inc.
...
def get_DLA_response_data_from_view(client_id, holding_id, banner_id, start_date, end_date): 
    query = f"""
    select 
...
def get_total_response_data_from_view(client_id, holding_id, banner_id, start_date, end_date): 
    query = f"""
    select 
        epos_organization_unit_num,
        response_id,
...
def get_DLA_measurement_effect(client_id, holding_id, banner_id, start_date, end_date):
    query = f"""  
          SELECT
          -- mdm_country_nm,
          -- mdm_holding_nm,
...
response_df = get_total_response_data_from_view(client_id, holding_id, banner_id, '2021-08-29', '2022-02-12')
...
measurement_df = get_total_measurement_effect(client_id, holding_id, banner_id, '2021-08-29', '2022-02-12')
...
def get_DLA_measured_effect(client_id, holding_id, banner_id, start_date, end_date):
    query = f"""  
          SELECT
          mdm_country_nm,
          mdm_holding_nm,
...
def get_total_measured_effect(client_id, holding_id, banner_id, start_date, end_date):
    query = f"""  
          SELECT
          mdm_country_nm,
          mdm_holding_nm,
...
measured_df_temp = get_total_measured_effect(client_id, holding_id, banner_id, '2021-08-29', '2022-02-12')
print(measured_df_temp.cache().count())

# COMMAND ----------

measured_df = get_total_measured_effect(client_id, holding_id, banner_id, '2021-06-01', '2022-12-31')
