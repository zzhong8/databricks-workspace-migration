...
# get reported retailer_item_ids
df_results = df_results.join(df_interventions.select('response_id', 'epos_retailer_item_id', 'SALES_DT'), ['response_id', 'SALES_DT'], how='left')
...
def compile_summary_table_udf(df: pd.DataFrame):
    ...
    retailer_item_id = df['epos_retailer_item_id'].unique()[0]
...
print(f'{df_summary_table.cache().count():,} Interventions measured')
...
df_summary_table_final = df_summary_table.filter(
    df_summary_table["is_complete"] == 'true'
)
print(f'{df_summary_table_final.cache().count():,} Interventions measured that have completed their measurement period')

# COMMAND ----------

# MAGIC %md
# MAGIC ## Finish Audit Table

# COMMAND ----------

# Load intermediate audit table for blob storage
df_stats = spark.read.format('delta').load(
    f'/mnt/processed/measurement/cache/{client_id}-{country_id}-{holding_id}-{banner_id}-audit-table-cache'
)

# Further pre-processing for audit table
step_4 = df_summary_table.select(
    pyf.col("response_id").alias("measured_responses")
)
step_5 = df_summary_table_final.select(
...
df_stats = (
    df_stats.join(
        step_4,
        df_stats.response_id == step_4.measured_responses,
        "left",
    )
    .join(
        step_5,
        df_stats.response_id == step_5.completed_measured_responses,
        "left",
    ).sort(("response_id"))
)
...
status_code_dictionary = {
    ...
    "2": "3 No matching ePOS data",
    "3": "0 Not measured due to model limitations",
    "4": "2 Measurement period not completed",
    "5": "1 Fully measured",
}
...
df_stats = df_stats.withColumn("status_code", mapping_expr[df_stats["status_code"]]).cache()
...
# To save space we drop all interventions that have a Non-measurable standard response text here
df_stats = df_stats.drop(*columns_to_drop).dropDuplicates().filter(
    df_stats["actionable_flg"].isNotNull()
).cache()
...
df_stats_pivot_table = (
    df_stats.groupBy("status_code")
    .agg(pyf.count("*").alias("count_of_status_code"))
    .orderBy(["status_code"], ascending=True)
)

display(df_stats_pivot_table)

# COMMAND ----------

client_nm = config_dict["mdm_client_nm"]
banner_nm = config_dict["mdm_banner_nm"]
country_nm = config_dict["mdm_country_nm"] 
...
df_stats.coalesce(1).write.format("csv").option("header", "true").mode(
    ...
    f"/mnt/processed/intervention_value_measurement/audit_tables/{country_nm}/{today_date}/{client_nm}-{banner_nm}-Intervention-Audit-Table-{today_date}"
)

# Write audit summary table to blob storage
df_stats_pivot_table.coalesce(1).write.format("csv").option("header", "true").mode(
    "overwrite"
).save(
    f"/mnt/processed/intervention_value_measurement/audit_tables/{country_nm}/{today_date}/{client_nm}-{banner_nm}-Intervention-Audit-Table-Summary-{today_date}"
)
...
''')
completed_response_ids = completed_response_ids.select(
    'response_id').distinct().rdd.flatMap(lambda x: x).collect()

# Filter out completed interventions
broadcast_unique_values = spark.sparkContext.broadcast(completed_response_ids)
schema = pyt.StructType(
    [pyt.StructField('response_id', pyt.StringType(), True)])
unique_values_df = spark.createDataFrame(
    [(value, ) for value in broadcast_unique_values.value], schema)
tmp = df_summary_table.join(unique_values_df, 'response_id', 'left_anti')
df_summary_table = tmp.select(df_summary_table.columns)
print(f'{df_summary_table.cache().count():,} New Interventions')
...
intervention_effects = df_results.filter('INTERVENTION_EFFECT != 0') \
    .select('UPC', 'INTERVENTION_EFFECT') \
    .groupby('UPC') \
    .mean().toPandas()
...
summary = df_results.groupby('UPC').agg(
    pyf.pow(pyf.stddev(pyf.col('POS_ITEM_QTY')), 2).alias('null_var'),
    pyf.pow(pyf.stddev(pyf.col('resid')), 2).alias('res_var'),
    pyf.mean('abs_resid').alias('MAE'),
...
# display(df_results.groupby('UPC', 'INTERVENTION_TYPE').mean())
agg_lift_results = df_results \
    ...
    .groupby('UPC', 'standard_response_cd', 'YEAR') \
    .agg(
...
display(
    df_results.withColumn('LIFT_PERCENT', pyf.col('INTERVENTION_EFFECT') / pyf.col('BASELINE')) \
        .select('UPC', 'standard_response_cd', 'POS_ITEM_QTY', 'BASELINE', 'EXPECTED',
                'INTERVENTION_EFFECT', 'LIFT_PERCENT') \
        .groupby('UPC', 'standard_response_cd') \
        .sum()
)
