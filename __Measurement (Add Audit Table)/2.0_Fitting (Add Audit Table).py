...
for cols in [predictor_columns, forecast_columns, required_columns]:
  if 'RETAILER_ITEM_ID' in cols:
    cols.append('UPC')
    cols.remove('RETAILER_ITEM_ID')
...
compress_data_schema = pyt.StructType([
    pyt.StructField('UPC', pyt.StringType()),
...
df_compressed = df_merged.select(required_columns).groupby('UPC').applyInPandas(
    compress_data,
...
def distributed_measurement(compressed_payload):
    ...
    try:
        indices_train, indices_val = train_test_split(
            data.index,
            test_size=0.1,
            stratify=data[['ORGANIZATION_UNIT_NUM', 'UPC']]
        )
    except ValueError:
        indices_train, indices_val = train_test_split(
            data.index,
    ...
    causal_kwargs = dict(
        ...
        learning_rate=0.3,
        loss_function='RMSE',
        cat_features=['ORGANIZATION_UNIT_NUM', 'UPC', 'DOW', intervention_col_label],
    ...
    try:
        ...
        print(f'==> Fitting failed on item = {upc} = {e}')
...
df_results = df_compressed.groupby('UPC').applyInPandas(
