...
min_date_filter = (today_date - relativedelta(months=15)).strftime(format='%Y-%m-%d')

# COMMAND ----------

df_test = spark.sql(f'select * from tesco_nestlecore_uk_dv.vw_sat_link_epos_summary')
df_test = df_test.where(pyf.col("SALES_DT") >= min_date_filter)

print(df_test.count())

df0 = spark.sql(f'select * from tesco_nestlecore_uk_dv.vw_sat_link_epos_summary')
df0 = df0.where(pyf.col("SALES_DT") >= min_date_filter)

retailer_items0 = spark.sql(f'''
    select RETAILER_ITEM_ID, HUB_RETAILER_ITEM_HK
    from tesco_nestlecore_uk_dv.hub_retailer_item
''')

stores_names0 = spark.sql(f'''
    select ORGANIZATION_UNIT_NUM, HUB_ORGANIZATION_UNIT_HK 
    from tesco_nestlecore_uk_dv.hub_organization_unit
''')

df0 = df0.join(
    retailer_items0,
    df0['HUB_RETAILER_ITEM_HK'] == retailer_items0['HUB_RETAILER_ITEM_HK'],
    'left_outer'
).drop(retailer_items0['HUB_RETAILER_ITEM_HK'])

df0 = df0.join(
    stores_names0,
    df0['HUB_ORGANIZATION_UNIT_HK'] == stores_names0['HUB_ORGANIZATION_UNIT_HK'],
    'left_outer'
).drop(stores_names0['HUB_ORGANIZATION_UNIT_HK'])

# retailer_items = spark.sql(f'''
#     select RETAILER_ITEM_ID, HUB_RETAILER_ITEM_HK
#     from tesco_nestlecore_uk_dv.hub_retailer_item
#     where RETAILER_ITEM_ID in (89922328, 71303980, 56859125, 89973647, 87740431)
# ''')

retailer_items = spark.sql(f'''
    select RETAILER_ITEM_ID, HUB_RETAILER_ITEM_HK
    from tesco_nestlecore_uk_dv.hub_retailer_item
    where RETAILER_ITEM_ID in (71303980)
''')
...
stores_names = spark.sql(f'''
    select ORGANIZATION_UNIT_NUM, HUB_ORGANIZATION_UNIT_HK 
    from tesco_nestlecore_uk_dv.hub_organization_unit
    where ORGANIZATION_UNIT_NUM in (2560)
''')
...
df_test = df_test.join(
    retailer_items,
    df_test['HUB_RETAILER_ITEM_HK'] == retailer_items['HUB_RETAILER_ITEM_HK'],
    'inner'
).drop(retailer_items['HUB_RETAILER_ITEM_HK'])
...
df_test = df_test.join(
    stores_names,
    df_test['HUB_ORGANIZATION_UNIT_HK'] == stores_names['HUB_ORGANIZATION_UNIT_HK'],
    'inner'
).drop(stores_names['HUB_ORGANIZATION_UNIT_HK'])
...
print(df0.where(pyf.col("SALES_DT") > today_date).count())

# COMMAND ----------

display(df0.where(pyf.col("SALES_DT") > today_date))

# COMMAND ----------

print(df0.where(pyf.col("ORGANIZATION_UNIT_NUM") == 2560).count())
print(df0.where(pyf.col("ORGANIZATION_UNIT_NUM") == 3333).count())

# COMMAND ----------

print(df0.where(pyf.col("RETAILER_ITEM_ID") == 89922328).count())
print(df0.where(pyf.col("RETAILER_ITEM_ID") == 71303980).count())
print(df0.where(pyf.col("RETAILER_ITEM_ID") == 56859125).count())
print(df0.where(pyf.col("RETAILER_ITEM_ID") == 89973647).count())
print(df0.where(pyf.col("RETAILER_ITEM_ID") == 87740431).count())
...
min_date_filter = (today_date - relativedelta(months=15)).strftime(format='%Y-%m-%d')
# min_date_filter = (today_date - datetime.timedelta(days=180)).strftime(format='%Y-%m-%d')
...
min_date_filter1 = (today_date - relativedelta(years=2)).strftime(format='%Y-%m-%d')
# min_date_filter = (today_date - datetime.timedelta(days=180)).strftime(format='%Y-%m-%d')

# COMMAND ----------

# Load POS data (with the nextgen processing function)
df_pos1 = get_pos_data(config_dict['epos_datavault_db_nm'], min_date_filter1, spark)

df_pos1 = df_pos1.fillna({
    'ON_HAND_INVENTORY_QTY': 0,
    'RECENT_ON_HAND_INVENTORY_QTY': 0,
    'RECENT_ON_HAND_INVENTORY_DIFF': 0
})

print(f'{df_pos1.cache().count():,}')

# COMMAND ----------

min_date_filter2 = (today_date - relativedelta(years=6)).strftime(format='%Y-%m-%d')
# min_date_filter = (today_date - datetime.timedelta(days=180)).strftime(format='%Y-%m-%d')

# COMMAND ----------

# Load POS data (with the nextgen processing function)
df_pos2 = get_pos_data(config_dict['epos_datavault_db_nm'], min_date_filter2, spark)

df_pos2 = df_pos2.fillna({
    'ON_HAND_INVENTORY_QTY': 0,
    'RECENT_ON_HAND_INVENTORY_QTY': 0,
    'RECENT_ON_HAND_INVENTORY_DIFF': 0
})

print(f'{df_pos2.cache().count():,}')
