...
df = (spark
      ...
      .csv("/mnt/training/bikeSharing/data-001/hour.csv")) # header and inferSchema both default to true
...
# MAGIC * weathersit: 
# MAGIC   * 1: Clear, Few clouds, Partly cloudy, Partly sunny
...
display(df.sample(withReplacement=False, fraction=0.1, seed=42))
...
df = df.drop('dteday', 'casual', 'registered', 'instant')
display(df)
...
featuresCols = df.columns[:-1] # Remove the true label from our features
...
pipeline = Pipeline().setStages([vectorAssembler, vectorIndexer, cv])
