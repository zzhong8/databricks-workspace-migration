...
# MAGIC %md
# MAGIC Use a [selectExpr](https://spark.apache.org/docs/latest/api/python/pyspark.sql.html#pyspark.sql.DataFrame.selectExpr) to extract `rbi` and `r` from `baseballDF` and rename them `features` and `label`, respectively. 
# MAGIC
# MAGIC Then create a temporary view with just these two features called `baseball` using [createOrReplaceTempView](https://spark.apache.org/docs/latest/api/python/pyspark.sql.html#pyspark.sql.DataFrame.createOrReplaceTempView).

# COMMAND ----------

# TODO: Replace <FILL_IN> with appropriate code
baseballDF.selectExpr('rbi as features', 'r as label').createOrReplaceTempView('baseball')

# selectExpr(df, "(max(Temp) - min(Temp)) as TempDiff")
...
print(baseballDF.corr("r", "rbi"))

# Option 2: Use the View
print(spark.table("baseball").corr("features", "label"))
...
# MAGIC
# MAGIC But first, we need to do some feature pre-processing. Let's read our `baseball` table using `spark.read.table()`, remove any rows that have missing values in either the `features` or `label` column, and assign the resulting DataFrame to the variable `cleanBaseballDF`.

# COMMAND ----------

cleanBaseballDF = spark.read.table("baseball").na.drop()

# COMMAND ----------

# MAGIC %md
# MAGIC Before we can use our `cleanBaseballDF` to build a linear regression model, we need to split our data into a training and test set to be able to evaluate our model at the end. We use the [randomSplit()](https://spark.apache.org/docs/latest/api/python/pyspark.sql.html#pyspark.sql.DataFrame.randomSplit) method to split our data, and include a seed for reproducibility.
# MAGIC
...
seed = 42
(testDF, trainingDF) = cleanBaseballDF.randomSplit((0.2, 0.8), seed=seed)

print(testDF.count(), trainingDF.count())
...
# MAGIC %md
# MAGIC Now that we have done all of our pre-processing, let's create a Linear Regression estimator with the default parameters using [LinearRegression()](https://spark.apache.org/docs/latest/api/python/pyspark.ml.html#pyspark.ml.regression.LinearRegression).
# MAGIC
...
display(predictionsAndLabelsDF.orderBy(predictionsAndLabelsDF.label.desc()))

# COMMAND ----------

# MAGIC %md
# MAGIC Now that we have built a model, we want to compute some metrics to evaluate how well our model performed. We will use SparkML's [RegressionEvaluator()](https://spark.apache.org/docs/latest/api/python/pyspark.ml.html#pyspark.ml.evaluation.RegressionEvaluator), and examine the different parameters by calling `explainParams()`.
