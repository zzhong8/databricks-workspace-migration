...
irisZeroIndexDF = irisDF.selectExpr('(label - 1) as label', 'features')
...
# MAGIC
# MAGIC One thing we need to be careful with K-means is that it only guarantees us a local optima - not a global optima. To be fairly confident that our local optima is also a global optima, we would need to re-run K-Means a few times with different initializations.
# MAGIC
