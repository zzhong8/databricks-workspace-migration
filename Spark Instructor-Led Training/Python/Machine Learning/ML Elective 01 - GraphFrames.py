...
csvSchema = StructType([
    StructField("Wk", IntegerType(), False),
    StructField("Date", StringType(), False),
    StructField("Time", StringType(), False),
    StructField("Day", StringType(), False),
    StructField("Winner", StringType(), False),
    StructField("Pts_W", IntegerType(), False),
    StructField("Location", StringType(), False),
    StructField("Loser", StringType(), False),
    StructField("Pts_L", IntegerType(), False),
    StructField("Rank_W", IntegerType(), False),
    StructField("Rank_L", IntegerType(), False)
])

# COMMAND ----------

csvFile = "dbfs:/mnt/training/301/football_2015.csv"

df = spark.read.csv(csvFile, header=True, schema=csvSchema)
display(df)
...
display(df.filter('Wk == 1').filter(col("Rank_W").isNotNull()).select(['Winner', 'Rank_W']))
...
winLossDF = g.inDegrees.join(g.outDegrees, 'id')
...
# MAGIC
# MAGIC You can verify the solution found by label by looking at the grouping of the West Coast schools (label 0 when using 5 iterations). 
# MAGIC
...
# TODO
shortest_path = g.shortestPaths(['Illinois'])
...
results = g.pageRank(maxIter = 8, resetProbability = 0.1)

# COMMAND ----------

rankingsDF = results.vertices.distinct().join(winLossDF, 'id')

display(rankingsDF.sort('pagerank', ascending=False).limit(10))
