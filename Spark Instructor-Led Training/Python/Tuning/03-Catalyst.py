...
# MAGIC - As built-in functions are integrated with the Catalyst, they can be optimized in ways that UDFs cannot. 
...
print(previous_threshold) # Default is 10MB
...
numbDF3 = spark.createDataFrame(map(lambda x: (x, ), range(1, 4)), ["n1"])
numbDF4 = spark.createDataFrame(map(lambda x: (x, ), range(4, 7)), ["n2"])

cartesianDF3 = numbDF3.crossJoin(numbDF4)
cartesianDF3.explain()
...
# MAGIC
# MAGIC **NOTE**: The Dataset API is not available in Python (it's still not available as of Spark 2.4), so this section is in Scala.
...
# MAGIC %scala
# MAGIC // TODO
# MAGIC // Rewrite this to use pure DSL and avoid closures.
# MAGIC personDS.filter(x => x.birthDate.split("-")(0).toInt > earliestYear) // everyone below 40
...
# MAGIC
# MAGIC personDS.filter(year($"birthDate") > earliestYear) // everyone born after the earliestYear (i.e. everyone below 40) 
# MAGIC         .filter($"salary" > 80000) // everyone earning more than 80K
...
# MAGIC  1. `left` is an <a href="https://github.com/apache/spark/blob/master/sql/catalyst/src/main/scala/org/apache/spark/sql/catalyst/expressions/namedExpressions.scala#L212">`AttributeReference`</a> which holds metadata and acts as a unique way of identifying the data
