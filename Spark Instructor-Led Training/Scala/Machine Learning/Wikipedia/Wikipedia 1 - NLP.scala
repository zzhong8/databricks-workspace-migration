...
printf("%d partitions%n", spark.read.parquet("dbfs:/mnt/wikipedia-readonly/en_wikipedia/march_5_snapshot/flattenedParquet_updated3days_30partitions/").rdd.partitions.length)
...
wikiDF.createOrReplaceTempView("wikipedia") // .registerTempTable() in Spark 1.x
...
val tempDF = spark.sql("SELECT title, lastrev_pdt_time FROM wikipedia LIMIT 10")
...
val hasWord = (s: String, w: String) => {
  val re = ("""\b""" + w + """\b""").r
  (s != null) && re.findFirstIn(s).isDefined
}
spark.udf.register("containsWord", hasWord)
...
assert(! hasWord("hello astronaut, how's space?", "test"))

// COMMAND ----------

// Look for the word 'space' in the first string
assert(hasWord("hello astronaut, how's space?", "space"))

// COMMAND ----------

assert(! hasWord("hello, spacey, how's it going up there?", "space"))
...
val wikiLoweredDF = wikiDF
  .select( $"*", lower($"text").as("lowerText") )
...
val tenPercentWordsListDF = tenPercentDF
  .select( explode($"words").as("word") )

tenPercentWordsListDF.printSchema
...
remover.transform(tenPercentDF)
  .select("id", "title", "words", "noStopWords")
  .show(50)

// COMMAND ----------

val noStopWordsListDF = remover
  .transform(tenPercentDF)
  .select( explode($"noStopWords").as("word") )
...
// TODO
val total = noStopWordsListDF
  // <fill-in>
  // <fill-in>
  .count

printf("%,d numbers%n", total)
