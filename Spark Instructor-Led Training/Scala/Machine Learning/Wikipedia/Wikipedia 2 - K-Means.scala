...
val parquetDir = "/mnt/wikipedia-readonly/en_wikipedia/march_5_snapshot/flattenedParquet_updated3days_30partitions/"
...
val wikiDF = spark.read
  ...
  .repartition(128)          // Repartition (8 works here, but we grow the data too much as we progress)
...
val model = PipelineModel.load("/mnt/wikipedia-readonly/models/5mill-articles-2.0")
...
val rawPredictionsDF = model.transform(wikiLoweredDF)
...
val predictionsDF = rawPredictionsDF.select("title", "prediction")
...
val apacheDF = predictionsDF
  .filter($"title".startsWith("Apache"))

apacheDF.show()

val apacheID = apacheDF
  .first()
  .getInt(1)
...
display(
  predictionsDF.filter("prediction = " + apacheID)
)
