...
val partialPipeline = new Pipeline().setStages(<FILL IN>) // Stages: tokenizer, remover, counts, binarizer

val pipelineModel = partialPipeline.<FILL IN>

val preppedDataDF = pipelineModel.<FILL IN>

// Previously, we put fitting a Logistic Regression model in the pipeline
val lrModel = new LogisticRegression().fit(<FILL IN>)
...
lr.setRegParam(<FILL_IN>)
counts.setVocabSize(<FILL_IN>)
val model = p.fit(trainDF)
val result = model.transform(testDF)
println("AUC: " + new BinaryClassificationEvaluator().<FILL_IN>)
