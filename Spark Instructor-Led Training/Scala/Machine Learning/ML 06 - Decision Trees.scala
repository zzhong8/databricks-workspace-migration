...
// MAGIC %md
// MAGIC Steps:
// MAGIC - [StringIndexer](https://spark.apache.org/docs/latest/api/scala/#org.apache.spark.ml.feature.StringIndexer) accepts an input column and converts the numerical values to categorical values. Set the `inputCol` to `label` and `outputCol` to `indexedLabel`.
// MAGIC - Set the `labelCol` of the [DecisionTreeClassifier](https://spark.apache.org/docs/latest/api/scala/#org.apache.spark.ml.classification.DecisionTreeClassifier) to `indexedLabel` (the column appended by `indexer`)
// MAGIC - Put these two steps together in a pipeline

// COMMAND ----------

// TODO: Replace <FILL_IN> with appropriate code
val indexer = new StringIndexer().setInputCol(<FILL_IN>).setOutputCol(<FILL_IN>)

val dtc = new DecisionTreeClassifier().setLabelCol(<FILL_IN>)

// Chain indexer + dtc together into a single ML Pipeline.
val pipeline = new Pipeline().setStages(<FILL_IN>)
...
val evaluator = new MulticlassClassificationEvaluator().setLabelCol("indexedLabel").setMetricName("accuracy")

// COMMAND ----------

// For each maxDepth setting, make predictions on the test data, and compute the accuracy metric.
val accuraciesDF = (0 until 8).map { maxDepth =>
