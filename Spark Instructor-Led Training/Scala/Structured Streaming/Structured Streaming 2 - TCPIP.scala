...
// MAGIC %md
// MAGIC Note that what just created is a regular DataFrame, with a single `value` column.
// MAGIC
...
// MAGIC > aggregations, aggregate values are maintained for each window the vent-time of a row falls into.
// MAGIC
