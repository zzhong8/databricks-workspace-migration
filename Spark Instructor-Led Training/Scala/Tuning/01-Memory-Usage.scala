...
val joinedDF = people2.join(names, names("firstName") === people2("firstName"))
val distDF = joinedDF.distinct.cache()
...
println("spark.default.parallelism: " + spark.sparkContext.defaultParallelism)
println(100.0 / df1.rdd.getNumPartitions)
