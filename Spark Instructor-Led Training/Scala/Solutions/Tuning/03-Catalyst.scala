...
val ipByCountry = transactionDF.join(ipRangesDF, transactionDF("ip_decimal") >= ipRangesDF("start") && transactionDF("ip_decimal") <= ipRangesDF("end"))
ipByCountry.show(5)
...
println("ipRangesDF: " + ipRangesDF.columns.length * ipRangesDF.count)
println("transactionDF: " + transactionDF.columns.length * transactionDF.count)

// broadcast the smaller df
val ipByCountryBroadcasted = ipRangesDF.join(broadcast(transactionDF), transactionDF("ip_decimal") >= ipRangesDF("start") && transactionDF("ip_decimal") <= ipRangesDF("end"))
...
personDS.filter(year($"birthDate") > earliestYear) // everyone above 40 
...
object DivideBy0Rule extends Rule[LogicalPlan] {
  override def apply(plan: LogicalPlan): LogicalPlan = {
    plan transformAllExpressions {
      case Divide(left,right) if right.asInstanceOf[Literal].value.asInstanceOf[Double] == 0 => {
        val lit = right.asInstanceOf[Literal].value.asInstanceOf[Double]
        // This is a logical plan, real data is not yet loaded in spark
        // for debug purposes, the best that can be done is to display the AttributeRef's 
        // toString (which yields most of the metadata about the cell) and the literal that 
        // is being used for division, in this case 0
        println("Division by 0 detected. Null will be returned. Operation: "+ left + "/" + lit)
        // ensure we return null, same as default spark behavior.
        Cast(Literal(null), NullType)
      }
    }
  }
}

spark.experimental.extraOptimizations = Seq(MultiplyBy0Rule, DivideBy0Rule)
