...
// MAGIC %md
// MAGIC That is both not surprising and fairly informative: the disparity in star distribution makes this result even more significant.
...
val tokenizedDF = tokenizer.transform(reviewsDF)
...
display(removedStopWordsDF.limit(1)) // Look at the first tokenized reviews without stop words
...
val p = new Pipeline().setStages(Array(tokenizer, remover, counts, binarizer, lr))
...
val model = p.fit(trainDF)
model.stages.last.asInstanceOf[LogisticRegressionModel].summary.asInstanceOf[BinaryLogisticRegressionSummary].areaUnderROC
...
// ANSWER
val partialPipeline = new Pipeline().setStages(Array(tokenizer, remover, counts, binarizer)) // Stages: tokenizer, remover, counts, binarizer

val pipelineModel = partialPipeline.fit(trainDF)

val preppedDataDF = pipelineModel.transform(trainDF)

// Previously, we put fitting a Logistic Regression model in the pipeline
val lrModel = new LogisticRegression().fit(preppedDataDF)
...
val model = p.fit(trainDF)
val result = model.transform(testDF)
println("AUC: " + new BinaryClassificationEvaluator().evaluate(result))
