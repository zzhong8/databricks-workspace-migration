...
input_data_list =  [('walmart', 'campbellssnack', 'us'), ('walmart', 'clorox', 'us'), ('walmart', 'tysonhillshire', 'us'),
                    ('walmart', 'rbusahealth', 'us'), ('walmart', 'rbusahygiene', 'us'), ('walmart', 'smuckers', 'ca'),
                    ('asda', 'generalmills', 'uk'), ('asda', 'kraftheinz', 'uk'), ('asda', 'nestlecereals', 'uk'), ('asda', 'premier', 'uk'),
                    ('morrisons', 'generalmills', 'uk'), ('morrisons', 'kraftheinz', 'uk'), ('morrisons', 'nestlecereals', 'uk'), ('morrisons', 'nestlecore', 'uk'), ('morrisons', 'premier', 'uk'),
                    ('tesco', 'generalmills', 'uk'), ('tesco', 'kraftheinz', 'uk'), ('tesco', 'nestlecereals', 'uk'), ('tesco', 'nestlecore', 'uk'), ('tesco', 'premier', 'uk'),
                   ('sainsburys', 'kraftheinz', 'uk'), ('sainsburys', 'nestlecereals', 'uk')]
...
input_data_list.append((retailer, client, country))
...
def build_alert_table(retailer, client, country_code):
    """Join alert table with lost sales table for all clients and add item_id to that.
    :param string retailer: the name of the retailer to pull
    :param string client: the name of the client to pull
    :return DataFrame
    """
    ...
    print('Build table for {}_{}'.format(retailer, client))
    sql_statement = """
        SELECT        
            HRI.RETAILER_ITEM_ID,
            \'{client}\' AS CLIENT,
            HOU.ORGANIZATION_UNIT_NUM,
            ASA.SALES_DT,
            NVL(ASA.LOST_SALES_AMT, 0) AS OPPORTUNITY_SALES_AMT,
            ASA.ALERT_MESSAGE_DESC,
            ASA.ON_HAND_INVENTORY_QTY
        FROM {database_im}.ALERT_ON_SHELF_AVAILABILITY ASA
        INNER JOIN {database_dv}.HUB_RETAILER_ITEM HRI
            ON ASA.HUB_RETAILER_ITEM_HK = HRI.HUB_RETAILER_ITEM_HK
        INNER JOIN {database_dv}.HUB_ORGANIZATION_UNIT HOU 
            ON ASA.HUB_ORGANIZATION_UNIT_HK = HOU.HUB_ORGANIZATION_UNIT_HK
        """
    sql_statement = sql_statement.format(retailer=retailer,
                                         client=client,
                                         country_code=country_code,
...
df_alert =  build_alert_table(retailer, client, country_code)
...
# ZORDER BY RETAILER_ITEM_ID, ORGANIZATION_UNIT_NUM
