...
# # Read POS data
# data_vault_data_pre_reset = data_vault_data.where((pyf.col("SALES_DT") < pyf.lit("2020-03-01")) & (pyf.col("SALES_DT") >= pyf.lit("2020-02-02")))
...
dbutils.widgets.text('PRE_RESET_START_DATE', '', '1 Pre-Reset Start Date')
dbutils.widgets.text('PRE_RESET_END_DATE', '', '2 Pre-Reset End Date')
dbutils.widgets.text('POST_RESET_START_DATE', '', '3 Post-Reset Start Date')
dbutils.widgets.text('POST_RESET_END_DATE', '', '4 Post-Reset End Date')
dbutils.widgets.dropdown('MODEL_SOURCE', 'prod', ['local', 'prod'], 'Model Source')

if len(dbutils.widgets.get('PRE_RESET_START_DATE')) > 0:
    pre_reset_start_date = datetime.datetime.strptime(dbutils.widgets.get('PRE_RESET_START_DATE'), '%Y%m%d')
else:
    pre_reset_start_date = None
    
if len(dbutils.widgets.get('PRE_RESET_END_DATE')) > 0:
    pre_reset_end_date = datetime.datetime.strptime(dbutils.widgets.get('PRE_RESET_END_DATE'), '%Y%m%d')
else:
    pre_reset_end_date = None
    
if len(dbutils.widgets.get('POST_RESET_START_DATE')) > 0:
    post_reset_start_date = datetime.datetime.strptime(dbutils.widgets.get('POST_RESET_START_DATE'), '%Y%m%d')
else:
    post_reset_start_date = None
    
if len(dbutils.widgets.get('POST_RESET_END_DATE')) > 0:
    post_reset_end_date = datetime.datetime.strptime(dbutils.widgets.get('POST_RESET_END_DATE'), '%Y%m%d')
else:
    post_reset_end_date = None
...
for param in [pre_reset_start_date, pre_reset_end_date, post_reset_start_date, post_reset_end_date]:
    print(param)
...
kroger_authorization_list_items_path = '/mnt/artifacts/hugh/kroger-starbucks/Business_Selected_Items.csv'
...
input_data_list =  [('kroger', 'starbucks', 'us')]
...
for retailer, client, country_code in input_data_list:
    if pre_reset_start_date == None:
        pre_reset_start_date = startDate_df[(startDate_df.RETAILER == retailer.upper()) & (startDate_df.CLIENT == client.upper()) \
            & (startDate_df.COUNTRY_CODE == country_code.upper())]['MAX_DATE'].values.astype('str')[0]    
    
    retailer_client_df = build_pos_table(
        retailer, client, country_code, pre_reset_start_date, pre_reset_end_date
    )
    ...
    unioned_df_pre_reset = unioned_df.unionAll(retailer_client_df)
...
for retailer, client, country_code in input_data_list:
    ...
    retailer_client_df = build_pos_table(
        retailer, client, country_code, post_reset_start_date, post_reset_end_date
    )
    ...
    unioned_df_post_reset = unioned_df.unionAll(retailer_client_df)

# COMMAND ----------

unioned_df_pre_reset = unioned_df_pre_reset.withColumn('RETAILER_ITEM_ID', pyf.regexp_replace('RETAILER_ITEM_ID', r'^[0]*', ''))
unioned_df_post_reset = unioned_df_post_reset.withColumn('RETAILER_ITEM_ID', pyf.regexp_replace('RETAILER_ITEM_ID', r'^[0]*', ''))
...
# join
result_pre_reset = unioned_df_pre_reset.join(df,
                         unioned_df_pre_reset.RETAILER_ITEM_ID == df['Product ID'],
                         how='inner') \
    .fillna(0, subset=df.columns[:-1]).drop('Product ID')

result_post_reset = unioned_df_post_reset.join(df,
                         unioned_df_post_reset.RETAILER_ITEM_ID == df['Product ID'],
                         how='inner') \
    .fillna(0, subset=df.columns[:-1]).drop('Product ID')

# COMMAND ----------

result_pre_reset.select('RETAILER_ITEM_ID').distinct().count()

# COMMAND ----------

result_post_reset.select('RETAILER_ITEM_ID').distinct().count()

# COMMAND ----------

display(result_pre_reset)

# COMMAND ----------

display(result_post_reset)

# COMMAND ----------

total_predicted_sales_by_date_pre_reset = result_pre_reset.select("SALES_DT", "DRFE_POS_ITEM_QTY").orderBy("SALES_DT").groupBy("SALES_DT").sum()

display(total_predicted_sales_by_date_pre_reset)

# COMMAND ----------

total_actual_sales_by_date_pre_reset = result_pre_reset.select("SALES_DT", "POS_ITEM_QTY").orderBy("SALES_DT").groupBy("SALES_DT").sum()

display(total_actual_sales_by_date_pre_reset)

# COMMAND ----------

total_dfre_error_by_date_pre_reset = result_pre_reset.select("SALES_DT", "DRFE_ERROR").orderBy("SALES_DT").groupBy("SALES_DT").sum()

display(total_dfre_error_by_date_pre_reset)

# COMMAND ----------

total_predicted_sales_by_date_post_reset = result_post_reset.select("SALES_DT", "DRFE_POS_ITEM_QTY").orderBy("SALES_DT").groupBy("SALES_DT").sum()

display(total_predicted_sales_by_date_post_reset)

# COMMAND ----------

total_actual_sales_by_date_post_reset = result_post_reset.select("SALES_DT", "POS_ITEM_QTY").orderBy("SALES_DT").groupBy("SALES_DT").sum()

display(total_actual_sales_by_date_post_reset)

# COMMAND ----------

total_dfre_error_by_date_post_reset = result_post_reset.select("SALES_DT", "DRFE_ERROR").orderBy("SALES_DT").groupBy("SALES_DT").sum()

display(total_dfre_error_by_date_post_reset)
