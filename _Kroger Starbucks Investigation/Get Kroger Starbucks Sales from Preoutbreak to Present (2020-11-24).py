...
sql_statement = """
    SELECT
       ...
       WHERE HRI.RETAILER_ITEM_ID = '002800046631' OR HRI.RETAILER_ITEM_ID = '002800046298'
"""

sql_statement = sql_statement.format(database=database, retailer=retailer, client=client, country_code=country_code)

data_vault_data1 = spark.sql(sql_statement)

# COMMAND ----------

# Read POS data
data_vault_data1 = data_vault_data1.where((pyf.col("SALES_DT") < pyf.lit("2021-01-01")) & (pyf.col("SALES_DT") >= pyf.lit("2020-01-01")))
...
items_assigned_with_business_rules_path = '/mnt/artifacts/hugh/kroger-starbucks/Kroger_Starbucks_Frequently_Alerted_Items.csv'
...
result.coalesce(1) \
    ...
    .save('/mnt/artifacts/hugh/kroger-starbucks/results')
