...
prediction_results_dataframe = prediction_results_dataframe.withColumn('CLIENT', pyf.lit(CLIENT))
prediction_results_dataframe = prediction_results_dataframe.withColumn('RETAILER', pyf.lit(RETAILER))

# COMMAND ----------

final_results_dataframe = prediction_results_dataframe \
    .select('CLIENT',
    'RETAILER',
    'ORGANIZATION_UNIT_NUM',
    'RETAILER_ITEM_ID',
...
final_results_dataframe.dtypes

# COMMAND ----------

# final_results_dataframe.count()

# COMMAND ----------

# display(final_results_dataframe)

# COMMAND ----------

# Import approved product list for this client
final_results_reference_path = '/mnt/artifacts/hugh/client={client}/retailer={retailer}/last_predicted_date={last_predicted_date}/'\
    .format(client=CLIENT, retailer=RETAILER, last_predicted_date=dbutils.widgets.get('LAST_DAY_TO_PREDICT_PARAM'))

# COMMAND ----------

print(final_results_reference_path)

# COMMAND ----------

# Save the data set
final_results_dataframe.coalesce(1) \
    .write.mode('overwrite').format('csv') \
    .save(final_results_reference_path)
