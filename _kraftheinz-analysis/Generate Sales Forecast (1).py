...
# PATHS (new!)
PATH_RESULTS_OUTPUT = '/mnt/artifacts/hugh/forecast-results/retailer={retailer}/client={client}/country_code={country_code}/last_predicted_date={last_predicted_date}'.format(
    retailer=RETAILER,
    client=CLIENT,
    country_code=COUNTRY_CODE,
    last_predicted_date=(LAST_PREDICTED_DATE).strftime('%Y-%m-%d')
)
...
prediction_results_dataframe.coalesce(1)\
    ...
    .option('header', 'true')\
    .mode('overwrite')\
    .save('{}'.format(PATH_RESULTS_OUTPUT))
