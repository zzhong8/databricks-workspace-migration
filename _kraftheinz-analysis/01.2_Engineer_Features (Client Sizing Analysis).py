...
retailer_item_ids = data.select('RETAILER_ITEM_ID').distinct()
retailer_item_ids.count()

# COMMAND ----------

organization_unit_nums = data.select('ORGANIZATION_UNIT_NUM').distinct()
organization_unit_nums.count()

# COMMAND ----------

sales_dates = data.select('SALES_DT').distinct()
sales_dates.count()
