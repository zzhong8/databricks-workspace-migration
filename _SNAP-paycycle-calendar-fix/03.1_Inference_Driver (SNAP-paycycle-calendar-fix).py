...
# 5 Ridge Regression
# 6 ElasticNet
from sklearn.linear_model import LinearRegression, LassoCV, RidgeCV, BayesianRidge, Ridge, ElasticNet
...
champions_path = '/mnt{mod}/artifacts/champion_models/retailer=walmart/client=dole_test'.format(mod='' if MODEL_SOURCE == 'LOCAL' else '/prod-ro')
...
store_with_default_snap_index = spark.read.format('csv')\
    .options(header='true', inferSchema='true')\
    .load('/mnt/artifacts/reference/snap_paycycle/retailer=walmart/client=dole_test/part-00000-tid-2510222031523584034-2b509ac8-34c8-4829-b5b0-b44e5e8f888f-1653074-c000.csv')

# COMMAND ----------

display(store_with_default_snap_index)

# COMMAND ----------

prediction_results_dataframe_sample = prediction_results_dataframe\
    .join(store_with_default_snap_index, prediction_results_dataframe.ORGANIZATION_UNIT_NUM == store_with_default_snap_index.ORGANIZATION_UNIT_NUM)\
    .select(prediction_results_dataframe['*'])

# COMMAND ----------

display(prediction_results_dataframe_sample)

# COMMAND ----------

prediction_results_dataframe_sample_subset = prediction_results_dataframe_sample.filter('MODEL_ID == "b20c5390-00cb-41d0-83f3-67927a9fcbe5"').toPandas()

# COMMAND ----------

display(prediction_results_dataframe_sample_subset)
