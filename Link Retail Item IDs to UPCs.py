...
sample_UPCs = sample_retail_item_IDs\
                         .join(sqlContext.read.table(productViewName).alias("PVN"),
                         pyf.col("_c0") == pyf.col("PVN.RefExternal"), "inner")\
                            .select("_c0", "PVN.ProductId").distinct()

sample_UPCs = sample_UPCs\
                         .join(sqlContext.read.table(productTableName).alias("PTN"),
                         pyf.col("PVN.ProductId") == pyf.col("PTN.ProductId"), "inner")\
                            .select("_c0", "PTN.InnerBarCode")
