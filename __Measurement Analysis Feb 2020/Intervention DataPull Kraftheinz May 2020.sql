...
select * from bobv2.Company where FullName like '%KRA%'
...
select * from bobv2.Campaign where CompanyID = '406'
...
select * from bobv2.surveyresponse sr where sr.VisitDate >= "2020-02-04"
...
select * from bobv2.callfile where CampaignId = '3263'

-- COMMAND ----------

select * from bobv2.callfile where CampaignId = '3447'

-- COMMAND ----------

select * from bobv2.callfile where CampaignId = '4165'

-- COMMAND ----------

select * from bobv2.callfile where CampaignId = '4511'

-- COMMAND ----------

select * from bobv2.callfile where CampaignId = '4700'
...
-- MAGIC                       AND COALESCE(pc.ChainId,c.ChainId) = r.ParentChainId \
-- MAGIC                     WHERE \
-- MAGIC                       cfv.CallfileId IN (19047, 19141, 19238, 19325, 19387, 19417) \
-- MAGIC                       AND ca.CampaignId = 3014 \
-- MAGIC                       AND cfv.IsDeleted = 0 \
-- MAGIC                       AND sr.IsDeleted = 0 \
-- MAGIC                       AND sir.IsDeleted = 0 \
...
-- MAGIC           InterventionNo = 1";
-- MAGIC
-- MAGIC dfIV_test = spark.sql(sql_test);
-- MAGIC # dfIV.createOrReplaceTempView("adv_analytics_IV")

-- COMMAND ----------

-- MAGIC %python
-- MAGIC display(dfIV_test.orderBy('InterventionDate'))

-- COMMAND ----------

-- MAGIC %python
-- MAGIC dfIV_test_dates = dfIV_test.select('InterventionDate')
-- MAGIC
-- MAGIC # Save the data set
-- MAGIC dfIV_test_dates.coalesce(1) \
-- MAGIC     .write.mode('overwrite').format('csv').option('header', 'true') \
-- MAGIC     .save('/mnt/processed/temp/intervention_dates-kraftheinz-new')

-- COMMAND ----------

-- MAGIC %python
-- MAGIC dfIV_test_dates.count()
...
-- MAGIC                       ON COALESCE(pcm.ProductId, psism.ProductId, 0) = r.ProductId \
-- MAGIC                       AND COALESCE(pc.ChainId,c.ChainId) = r.ParentChainId \
-- MAGIC                     WHERE ca.CampaignId = 3014 \
-- MAGIC                       AND cfv.IsDeleted = 0 \
-- MAGIC                       AND sr.IsDeleted = 0 \
-- MAGIC                       AND sir.IsDeleted = 0 \
-- MAGIC                       AND sir.ResponseValue IS NOT NULL) a) i \
...
-- MAGIC           InterventionNo = 1";
-- MAGIC
-- MAGIC dfIV_test2 = spark.sql(sql_test2);
-- MAGIC # dfIV.createOrReplaceTempView("adv_analytics_IV")

-- COMMAND ----------

-- MAGIC %python
-- MAGIC dfIV_test_dates2 = dfIV_test2.select('InterventionDate').distinct()
-- MAGIC
-- MAGIC # Save the data set
-- MAGIC dfIV_test_dates2.coalesce(1) \
-- MAGIC     .write.mode('overwrite').format('csv').option('header', 'true') \
-- MAGIC     .save('/mnt/processed/temp/intervention_dates-kraftheinz-new2')

-- COMMAND ----------

-- MAGIC %python
-- MAGIC display(dfIV_test)
...
-- MAGIC %python
-- MAGIC # RUN ME
-- MAGIC df_iv = spark.sql('select * from adv_analytics_IV where ParentChainName = \'Asda - Parent\'')
...
-- MAGIC
-- MAGIC df_iv.write.mode('overwrite').format('delta').save('/mnt/processed/temp/intervention_data-kraftheinz-asda')
...
-- MAGIC
-- MAGIC sql = "SELECT \
-- MAGIC           \"asda_kraftheinz_uk\" AS RETAIL_CLIENT \
...
-- MAGIC         FROM asda_kraftheinz_uk_dv.vw_sat_link_epos_summary s \
-- MAGIC         INNER JOIN asda_kraftheinz_uk_dv.hub_retailer_item ri \
-- MAGIC           ON s.HUB_RETAILER_ITEM_HK = ri.HUB_RETAILER_ITEM_HK \
-- MAGIC         INNER JOIN asda_kraftheinz_uk_dv.hub_organization_unit ou \
-- MAGIC           ON s.HUB_ORGANIZATION_UNIT_HK = ou.HUB_ORGANIZATION_UNIT_HK \
-- MAGIC         LEFT JOIN asda_kraftheinz_uk_retail_alert_im.vw_drfe_forecast_baseline_unit d \
...
-- MAGIC
-- MAGIC df_ivs_with_epos.write.mode('overwrite').format('delta').save('/mnt/processed/temp/kraftheinz_asda_inv_epos')
...
-- MAGIC # print(df_iv.count())
-- MAGIC
-- MAGIC df_iv.write.mode('overwrite').format('delta').save('/mnt/processed/temp/intervention_data-kraftheinz-tesco')
...
-- MAGIC
-- MAGIC sql = "SELECT \
-- MAGIC           \"tesco_kraftheinz_uk\" AS RETAIL_CLIENT \
...
-- MAGIC           , ou.ORGANIZATION_UNIT_NUM \
-- MAGIC           , d.BASELINE_POS_ITEM_QTY \
-- MAGIC         FROM tesco_kraftheinz_uk_dv.vw_sat_link_epos_summary s \
-- MAGIC         INNER JOIN tesco_kraftheinz_uk_dv.hub_retailer_item ri \
-- MAGIC           ON s.HUB_RETAILER_ITEM_HK = ri.HUB_RETAILER_ITEM_HK \
-- MAGIC         INNER JOIN tesco_kraftheinz_uk_dv.hub_organization_unit ou \
-- MAGIC           ON s.HUB_ORGANIZATION_UNIT_HK = ou.HUB_ORGANIZATION_UNIT_HK \
-- MAGIC         LEFT JOIN tesco_kraftheinz_uk_retail_alert_im.vw_drfe_forecast_baseline_unit d \
-- MAGIC         /* This could be replaced with expected baseline table */ \
-- MAGIC           ON s.SALES_DT = d.SALES_DT \
-- MAGIC           AND s.HUB_ORGANIZATION_UNIT_HK = d.HUB_ORGANIZATION_UNIT_HK \
-- MAGIC           AND s.HUB_RETAILER_ITEM_HK = d.HUB_RETAILER_ITEM_HK"
...
-- MAGIC df_ivs_with_epos = spark.sql('select * from ivs_with_epos')
-- MAGIC
-- MAGIC df_ivs_with_epos.write.mode('overwrite').format('delta').save('/mnt/processed/temp/kraftheinz_tesco_inv_epos')

-- COMMAND ----------

-- MAGIC %python
-- MAGIC # RUN ME
-- MAGIC df_iv = spark.sql('select * from adv_analytics_IV where ParentChainName = \'Morrisons - Parent\'')
...
-- MAGIC # print(df_iv.count())
-- MAGIC
-- MAGIC df_iv.write.mode('overwrite').format('delta').save('/mnt/processed/temp/intervention_data-kraftheinz-morrisons')
...
-- MAGIC
-- MAGIC sql = "SELECT \
-- MAGIC           \"morrisons_kraftheinz_uk\" AS RETAIL_CLIENT \
...
-- MAGIC           , ou.ORGANIZATION_UNIT_NUM \
-- MAGIC           , d.BASELINE_POS_ITEM_QTY \
-- MAGIC         FROM morrisons_kraftheinz_uk_dv.vw_sat_link_epos_summary s \
-- MAGIC         INNER JOIN morrisons_kraftheinz_uk_dv.hub_retailer_item ri \
-- MAGIC           ON s.HUB_RETAILER_ITEM_HK = ri.HUB_RETAILER_ITEM_HK \
-- MAGIC         INNER JOIN morrisons_kraftheinz_uk_dv.hub_organization_unit ou \
-- MAGIC           ON s.HUB_ORGANIZATION_UNIT_HK = ou.HUB_ORGANIZATION_UNIT_HK \
-- MAGIC         LEFT JOIN morrisons_kraftheinz_uk_retail_alert_im.vw_drfe_forecast_baseline_unit d \
-- MAGIC         /* This could be replaced with expected baseline table */ \
-- MAGIC           ON s.SALES_DT = d.SALES_DT \
-- MAGIC           AND s.HUB_ORGANIZATION_UNIT_HK = d.HUB_ORGANIZATION_UNIT_HK \
-- MAGIC           AND s.HUB_RETAILER_ITEM_HK = d.HUB_RETAILER_ITEM_HK"
...
-- MAGIC df_ivs_with_epos = spark.sql('select * from ivs_with_epos')
-- MAGIC
-- MAGIC df_ivs_with_epos.write.mode('overwrite').format('delta').save('/mnt/processed/temp/kraftheinz_morrisons_inv_epos')

-- COMMAND ----------

-- MAGIC %python
-- MAGIC # RUN ME
-- MAGIC df_iv = spark.sql('select * from adv_analytics_IV where ParentChainName = \'Sainsburys - Parent\'')
...
-- MAGIC # print(df_iv.count())
-- MAGIC
-- MAGIC df_iv.write.mode('overwrite').format('delta').save('/mnt/processed/temp/intervention_data-kraftheinz-sainsburys')
...
-- MAGIC
-- MAGIC sql = "SELECT \
-- MAGIC           \"sainsburys_kraftheinz_uk\" AS RETAIL_CLIENT \
...
-- MAGIC           , ou.ORGANIZATION_UNIT_NUM \
-- MAGIC           , d.BASELINE_POS_ITEM_QTY \
-- MAGIC         FROM sainsburys_kraftheinz_uk_dv.vw_sat_link_epos_summary s \
-- MAGIC         INNER JOIN sainsburys_kraftheinz_uk_dv.hub_retailer_item ri \
-- MAGIC           ON s.HUB_RETAILER_ITEM_HK = ri.HUB_RETAILER_ITEM_HK \
-- MAGIC         INNER JOIN sainsburys_kraftheinz_uk_dv.hub_organization_unit ou \
-- MAGIC           ON s.HUB_ORGANIZATION_UNIT_HK = ou.HUB_ORGANIZATION_UNIT_HK \
-- MAGIC         LEFT JOIN sainsburys_kraftheinz_uk_retail_alert_im.vw_drfe_forecast_baseline_unit d \
-- MAGIC         /* This could be replaced with expected baseline table */ \
-- MAGIC           ON s.SALES_DT = d.SALES_DT \
-- MAGIC           AND s.HUB_ORGANIZATION_UNIT_HK = d.HUB_ORGANIZATION_UNIT_HK \
-- MAGIC           AND s.HUB_RETAILER_ITEM_HK = d.HUB_RETAILER_ITEM_HK"
...
-- MAGIC df_ivs_with_epos = spark.sql('select * from ivs_with_epos')
-- MAGIC
-- MAGIC df_ivs_with_epos.write.mode('overwrite').format('delta').save('/mnt/processed/temp/kraftheinz_sainsburys_inv_epos')
