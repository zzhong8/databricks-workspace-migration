...
select * from bobv2.Company where FullName like '%MILL%'
...
select * from bobv2.Campaign where CompanyID = '463'
...
--General Mills UK
select * from bobv2.callfile where CampaignId = '4237'
...
-- MAGIC                       AND COALESCE(pc.ChainId,c.ChainId) = r.ParentChainId \
-- MAGIC                     WHERE \
-- MAGIC                       cfv.CallfileId IN (19047, 19141) \
...
-- MAGIC %python
-- MAGIC dfIV_dates = dfIV.select('InterventionDate')
-- MAGIC
-- MAGIC # Save the data set
-- MAGIC dfIV_dates.coalesce(1) \
-- MAGIC     .write.mode('overwrite').format('csv').option('header', 'true') \
-- MAGIC     .save('/mnt/processed/temp/intervention_dates-generalmills')

-- COMMAND ----------

-- MAGIC %python
-- MAGIC dfIV_dates.count()
