...
display(df_spark.select(['INTERVENTION_TYPE', 'CALLFILE_VISIT_ID', 'INTERVENTION_ID', 'BASELINE', 'INTERVENTION_EFFECT']).groupby('INTERVENTION_TYPE', 'CALLFILE_VISIT_ID', 'INTERVENTION_ID').sum('BASELINE', 'INTERVENTION_EFFECT'))

# COMMAND ----------

df_intervention = df_spark.select(['INTERVENTION_TYPE', 'CALLFILE_VISIT_ID', 'INTERVENTION_ID', 'BASELINE', 'INTERVENTION_EFFECT']).groupby('INTERVENTION_TYPE', 'CALLFILE_VISIT_ID', 'INTERVENTION_ID').sum('BASELINE', 'INTERVENTION_EFFECT').withColumnRenamed('sum(BASELINE)', 'SUMMED_BASELINE').withColumnRenamed('sum(INTERVENTION_EFFECT)', 'SUMMED_INTERVENTION_EFFECT')

# COMMAND ----------

df_intervention.columns
...
display(df_intervention.groupby('INTERVENTION_TYPE').sum('SUMMED_INTERVENTION_EFFECT'))

# COMMAND ----------

display(df_intervention.groupby('INTERVENTION_TYPE').count())

# COMMAND ----------

df_intervention_adjusted = df_intervention.withColumn('SUMMED_INTERVENTION_EFFECT', pyf.when(pyf.col('SUMMED_INTERVENTION_EFFECT') <= 0, 0).otherwise(pyf.col('SUMMED_INTERVENTION_EFFECT')))

display(df_intervention_adjusted)

# COMMAND ----------

display(df_intervention_adjusted.groupby('INTERVENTION_TYPE').sum('SUMMED_INTERVENTION_EFFECT'))
...
median_finder = pyf.udf(find_median, pyt.FloatType())

# COMMAND ----------

intervention_effect_list = df_intervention_adjusted.groupBy("INTERVENTION_TYPE").agg(pyf.collect_list("SUMMED_INTERVENTION_EFFECT").alias("SUMMED_INTERVENTION_EFFECTS"))

intervention_effect_list = intervention_effect_list.withColumn("median", median_finder("SUMMED_INTERVENTION_EFFECTS")) 

# COMMAND ----------

display(intervention_effect_list.select('INTERVENTION_TYPE', 'median'))

# COMMAND ----------

display(df_intervention_adjusted.groupby('INTERVENTION_TYPE').mean('SUMMED_INTERVENTION_EFFECT'))

# COMMAND ----------

display(df_intervention_adjusted.groupby('INTERVENTION_TYPE').count())

# COMMAND ----------

# display(df_intervention_adjusted.groupby('INTERVENTION_TYPE').count('SUMMED_INTERVENTION_EFFECT').sum('SUMMED_INTERVENTION_EFFECT'))

# COMMAND ----------

display(df_spark.groupby('INTERVENTION_TYPE').count())
...
df_adjusted = df_intervention_adjusted.toPandas()

# COMMAND ----------

results, results_lift, results_lift_total = 3*[np.zeros((2, 2))]
print(f'**{retailer.upper()}** - All Interventions')
for i, (target, rule) in enumerate(itertools.product(['SUMMED_INTERVENTION_EFFECT'], ['All'])):
    if rule.lower() == 'all':
        estimate = df_adjusted[target].sum()
    else:
        estimate = np.maximum(df_adjusted[target], 0).sum()
        
    lift = estimate / df_adjusted['SUMMED_BASELINE'].sum()
