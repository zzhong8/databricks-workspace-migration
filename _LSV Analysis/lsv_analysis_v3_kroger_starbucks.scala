...
// MAGIC %sql
// MAGIC select * from kroger_starbucks_us_retail_alert_im.alert_inventory_cleanup limit 10
...
// MAGIC
// MAGIC     FROM kroger_starbucks_us_dv.vw_sat_link_epos_summary vw_sat_lnk 
// MAGIC     INNER JOIN kroger_starbucks_us_dv.VW_SAT_RETAILER_ITEM_UNIT_PRICE vw_sat_unt_prc
...
// MAGIC     
// MAGIC     INNER JOIN kroger_starbucks_us_retail_alert_im.DRFE_FORECAST_BASELINE_UNIT fcst_unt 
...
WITH CALC_LSV AS 
   (
    ...
    FROM kroger_starbucks_us_dv.vw_sat_link_epos_summary vw_sat_lnk 
    INNER JOIN kroger_starbucks_us_dv.VW_SAT_RETAILER_ITEM_UNIT_PRICE vw_sat_unt_prc
    ...
    INNER JOIN kroger_starbucks_us_retail_alert_im.DRFE_FORECAST_BASELINE_UNIT fcst_unt 
...
spark.sql(viewDdl).createOrReplaceTempView("tmp_vw_kroger_starbucks_lost_sales_value_new")
...
// MAGIC %sql
// MAGIC select * from tmp_vw_kroger_starbucks_lost_sales_value_new
// MAGIC where
// MAGIC SALES_DT = date('2020-09-01')
// MAGIC limit 100
...
// MAGIC from
// MAGIC
// MAGIC kroger_starbucks_us_retail_alert_im.LOST_SALES_VALUE old_lsv,
// MAGIC tmp_vw_kroger_starbucks_lost_sales_value_new new_lsv
// MAGIC
// MAGIC where
// MAGIC
...
// MAGIC count(*)
// MAGIC from
// MAGIC kroger_starbucks_us_retail_alert_im.LOST_SALES_VALUE old_lsv inner join
// MAGIC tmp_vw_kroger_starbucks_lost_sales_value_new new_lsv
// MAGIC
// MAGIC on
// MAGIC old_lsv.sales_dt = new_lsv.sales_dt and
...
// MAGIC count(*)
// MAGIC from
// MAGIC kroger_starbucks_us_retail_alert_im.LOST_SALES_VALUE old_lsv left outer join
// MAGIC tmp_vw_kroger_starbucks_lost_sales_value_new new_lsv
// MAGIC
// MAGIC on
// MAGIC old_lsv.sales_dt = new_lsv.sales_dt and
...
// MAGIC count(*)
// MAGIC from
// MAGIC kroger_starbucks_us_retail_alert_im.LOST_SALES_VALUE old_lsv right outer join
// MAGIC tmp_vw_kroger_starbucks_lost_sales_value_new new_lsv
// MAGIC
// MAGIC on
// MAGIC old_lsv.sales_dt = new_lsv.sales_dt and
...
FROM kroger_starbucks_us_dv.vw_sat_link_epos_summary sles

INNER JOIN (select * from tmp_vw_kroger_starbucks_lost_sales_value_new ) llsv -- Pulling data from the new LSV derivation TMP View
...
INNER JOIN (select * from tmp_vw_kroger_starbucks_lost_sales_value_new ) adsd -- Pulling data from the new LSV derivation TMP View
...
"""
spark.sql(sqlOsaSlowSales).createOrReplaceTempView("tmp_vw_kroger_starbucks_alert_on_shelf_availability_new")

// COMMAND ----------

// MAGIC %sql
// MAGIC select * from tmp_vw_kroger_starbucks_alert_on_shelf_availability_new
// MAGIC where
// MAGIC SALES_DT between date('2020-09-01') and date('2020-09-10')
// MAGIC and
// MAGIC (
...
// MAGIC   ALERT_MESSAGE_DESC,
// MAGIC   count(*)
// MAGIC from tmp_vw_kroger_starbucks_alert_on_shelf_availability_new
// MAGIC where
// MAGIC   SALES_DT between date('2020-06-01') and date('2020-09-05') and
// MAGIC   ALERT_MESSAGE_DESC = 'Slow Sales' and
// MAGIC   LOST_SALES_AMT >= 7
...
// MAGIC from
// MAGIC
// MAGIC kroger_starbucks_us_retail_alert_im.alert_on_shelf_availability old_osa
// MAGIC inner join 
// MAGIC tmp_vw_kroger_starbucks_alert_on_shelf_availability_new new_osa
// MAGIC
// MAGIC on
// MAGIC
...
// MAGIC from kroger_starbucks_us_retail_alert_im.alert_on_shelf_availability
// MAGIC where
// MAGIC   SALES_DT between date('2020-08-01') and date('2020-09-15') 
...
//   ALERT_MESSAGE_DESC,
//   count(*)
// from danonewave_drt_us_team_alerts_im.alert_on_shelf_availability
// where
//   SALES_DT between date('2019-01-01') and date('2020-09-15') 
//   and ALERT_MESSAGE_DESC <> 'Slow Sales'
// group by 
//   sales_dt,
//   ALERT_MESSAGE_DESC
// order by
...
//   ALERT_MESSAGE_DESC,
//   count(*)
// from walmart_nestlewaters_us_retail_alert_im.alert_on_shelf_availability
// where
//   SALES_DT between date('2019-01-01') and date('2020-09-15') 
//   and ALERT_MESSAGE_DESC <> 'Slow Sales'
// group by 
...
// MAGIC from starbucks_drt_us_team_alerts_im.alert_on_shelf_availability

// COMMAND ----------

// MAGIC %sql
// MAGIC select * from kroger_starbucks_us_dv.SAT_LINK_BOH 
...
// MAGIC from kroger_starbucks_us_dv.SAT_LINK_BOH 
// MAGIC where
