...
SELECT
  ...
  , llsv.SALES_DT
  , SUM(llsv.LOST_SALES_AMT) AS LOST_SALES_AMT_OLD
  
--, (SELECT sum(adsd.LOST_SALES_AMT) FROM tmp_vw_kroger_danonewave_lost_sales_value_new adsd
--      where adsd.SALES_DT between llsv.SALES_DT - interval 1 day and llsv.SALES_DT) as LOST_SALES_AMT_2
  
  , sum(sum(llsv.LOST_SALES_AMT)) over (order by llsv.HUB_ORGANIZATION_UNIT_HK, llsv.HUB_RETAILER_ITEM_HK, llsv.SALES_DT rows between 1 preceding and CURRENT ROW) as LOST_SALES_AMT
...
// MAGIC from kroger_danonewave_us_retail_alert_im.alert_on_shelf_availability
// MAGIC where
// MAGIC   SALES_DT between date('2020-09-01') and date('2020-09-22') and
...
// MAGIC old_osa.HUB_Retailer_Item_HK = new_osa.HUB_Retailer_Item_HK and
// MAGIC
// MAGIC old_osa.sales_dt between date('2020-09-01') and date('2020-09-22') and
// MAGIC old_osa.ALERT_MESSAGE_DESC = 'Slow Sales' -- Pulling only Slow Sales Alerts from the OLD OSA table
// MAGIC
// MAGIC group by 
// MAGIC   old_osa.sales_dt,
// MAGIC   old_osa.ALERT_MESSAGE_DESC
...
// MAGIC %sql
// MAGIC select * from kroger_danonewave_us_dv.sat
