...
dbutils.widgets.text('run_id', 'auto', 'Run ID')
...
# Get params
SOURCE_SYSTEM = dbutils.widgets.get('source_system').strip().casefold()
RETAILER = dbutils.widgets.get('retailer').strip().casefold()
CLIENT = dbutils.widgets.get('client').strip().casefold()
COUNTRY_CODE = dbutils.widgets.get('country_code').strip().casefold()
...
RUN_ID = dbutils.widgets.get('run_id').strip().casefold()
...
def compress_pos_data(df):
    print(f'--> Compressing {df["RETAILER_ITEM_ID"].unique()}')
    return pd.DataFrame(
        [[
...
df_compressed = data_partitioned.groupby('RETAILER_ITEM_ID').applyInPandas(
    compress_pos_data,
    schema=compress_pos_data_schema
)
...
databaseName = '{}_{}_{}_{}_dv'.format(SOURCE_SYSTEM.lower(), RETAILER.lower(), CLIENT.lower(), COUNTRY_CODE.lower())
itemMasterTableName = '{}.vw_latest_sat_retailer_item'.format(databaseName)
storeMasterTableName = '{}.vw_latest_sat_organization_unit'.format(databaseName)
...
if ENVIRONMENT == 'dev':
    ...
    insertDatabaseName = 'retail_alert_{}_{}_{}_im'.format(RETAILER.lower(), CLIENT.lower(), COUNTRY_CODE.lower())
