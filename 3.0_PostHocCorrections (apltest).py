...
df_results = spark.read.format('delta').load(
    f'/mnt/processed/measurement/cache/{client_id}-{country_id}-{holding_id}-{banner_id}-processed-apltest'
)
total_baseline = df_results.agg(pyf.sum('BASELINE')).toPandas().values[0][0]

df_interventions = spark.read.format('delta').load(
    f'/mnt/processed/measurement/cache/{client_id}-{country_id}-{holding_id}-{banner_id}-intervention-apltest'
)
...
# MAGIC %sql
# MAGIC create table if not exists default.ds_intervention_summary_apltest
# MAGIC (
...
# MAGIC LOCATION 'abfss://data@eus2psag2dpcoredatalake.dfs.core.windows.net/informationmart/acosta_retail_report/ds_intervention_summary_apltest'
...
df_results \
    ...
    .format('delta') \
    .mode('overwrite') \
    .save(f'/mnt/processed/measurement/results/{client_id}-{country_id}-{holding_id}-{banner_id}-apltest')
...
df_summary_table \
    ...
    .format('delta') \
    .mode('overwrite') \
    .save(f'/mnt/processed/measurement/results/{client_id}-{country_id}-{holding_id}-{banner_id}-summary-apltest')
...
completed_response_ids = spark.sql(f'''
    ...
    from default.ds_intervention_summary_apltest
    where is_complete = true
    and mdm_client_id = {client_id}
''')
completed_response_ids = completed_response_ids.select('response_id').distinct().rdd.flatMap(lambda x: x).collect()

# Filter out completed interventions
broadcast_unique_values = spark.sparkContext.broadcast(completed_response_ids)
schema = pyt.StructType([pyt.StructField('response_id', pyt.StringType(), True)])
unique_values_df = spark.createDataFrame([(value,) for value in broadcast_unique_values.value], schema)
