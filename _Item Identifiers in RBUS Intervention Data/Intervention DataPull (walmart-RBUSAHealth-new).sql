...
-- MAGIC                                   , REPLACE(REPLACE(REPLACE(pmc.RefExternal, CHAR(9), ''), CHAR(10), ''), CHAR(13), '') AS RefExternal \
...
-- MAGIC           AND parentChainName = \'Walmart - Parent\'";
-- MAGIC
...
-- MAGIC df_iv = spark.sql('select * from adv_analytics_IV')
...
-- MAGIC %python
-- MAGIC retailer_item_ids_and_organization_unit_nums_df_iv = df_iv.select("RefExternal", "ChainRefExternal").distinct()

-- COMMAND ----------

-- MAGIC %python
-- MAGIC # RBUS_HYHO_Non_Matched_Item_List = spark.read.format('csv')\
-- MAGIC #     .options(header='true', inferSchema='true')\
-- MAGIC #     .load('/mnt/processed/temp/csv/RBUS HYHO Non-Matched Item List.csv')

-- COMMAND ----------

-- MAGIC %python
-- MAGIC # RBUS_HYHO_Non_Matched_Item_List.count()

-- COMMAND ----------

-- MAGIC %python
-- MAGIC # retailer_item_ids_df_iv_and_organization_unit_nums_debug2 = retailer_item_ids_and_organization_unit_nums_df_iv\
-- MAGIC #     .join(RBUS_HYHO_Non_Matched_Item_List, 'RefExternal', 'inner')
-- MAGIC
-- MAGIC # display(retailer_item_ids_df_iv_and_organization_unit_nums_debug2)

-- COMMAND ----------

-- MAGIC %python
-- MAGIC retailer_item_ids_df_iv.cache()

-- COMMAND ----------

-- MAGIC %python
-- MAGIC from pyspark.sql import types as pyt
-- MAGIC from pyspark.sql import functions as pyf
-- MAGIC
-- MAGIC # retailer_item_ids_df_iv_debug = df_iv.select("RefExternal", "ChainRefExternal").distinct().where(pyf.col("RefExternal") == 1302949)
-- MAGIC
-- MAGIC # display(retailer_item_ids_df_iv_debug)
...
-- MAGIC %python
-- MAGIC
-- MAGIC # retailer_item_ids_df_iv.write.mode('overwrite').format('delta').save('/mnt/processed/temp/intervention_data-rbusahealth-walmart-retailer_item_ids')
...
-- MAGIC %python
-- MAGIC # display(RBUS_HYHO_Non_Matched_Item_List)

-- COMMAND ----------

-- MAGIC %python
-- MAGIC # retailer_item_ids_and_organization_unit_nums_dfEPOS = dfEPOS.select("RETAILER_ITEM_ID", "ORGANIZATION_UNIT_NUM").distinct()

-- COMMAND ----------

-- MAGIC %python
-- MAGIC # display(retailer_item_ids_and_organization_unit_nums_dfEPOS)

-- COMMAND ----------

-- MAGIC %python
-- MAGIC # retailer_item_ids_and_organization_unit_nums_dfEPOS_debug2 = retailer_item_ids_and_organization_unit_nums_dfEPOS\
-- MAGIC #     .join(RBUS_HYHO_Non_Matched_Item_List, retailer_item_ids_and_organization_unit_nums_dfEPOS.RETAILER_ITEM_ID == RBUS_HYHO_Non_Matched_Item_List.RefExternal, 'inner')\
-- MAGIC #     .drop('RefExternal')
-- MAGIC
-- MAGIC # display(retailer_item_ids_and_organization_unit_nums_dfEPOS_debug2)

-- COMMAND ----------

-- MAGIC %python
-- MAGIC # df_joined = retailer_item_ids_df_iv_and_organization_unit_nums_debug2\
-- MAGIC #     .join(retailer_item_ids_and_organization_unit_nums_dfEPOS_debug2,\
-- MAGIC #     (retailer_item_ids_df_iv_and_organization_unit_nums_debug2.RefExternal == retailer_item_ids_and_organization_unit_nums_dfEPOS_debug2.RETAILER_ITEM_ID) &\
-- MAGIC #     (retailer_item_ids_df_iv_and_organization_unit_nums_debug2.ChainRefExternal == retailer_item_ids_and_organization_unit_nums_dfEPOS_debug2.ORGANIZATION_UNIT_NUM),\
-- MAGIC #     'inner')
-- MAGIC
...
-- MAGIC %python
-- MAGIC # dfEPOS_debug = dfEPOS.select("RETAILER_ITEM_ID", "ORGANIZATION_UNIT_NUM").where(pyf.col("RETAILER_ITEM_ID") == 1302949).distinct()
-- MAGIC
-- MAGIC # display(dfEPOS_debug)
...
CREATE OR REPLACE TEMPORARY VIEW ivs_with_epos_temp
AS SELECT 
          iv.CallfileId
          , iv.CallfileVisitId
          , iv.InterventionCount
          ...
          , s.BASELINE_POS_ITEM_QTY
        FROM adv_analytics_IV iv
        JOIN adv_analytics_IV_epos_sales s
          ON iv.RefExternal = s.RETAILER_ITEM_ID

-- COMMAND ----------

-- MAGIC %python
-- MAGIC # RUN ME
-- MAGIC df_ivs_with_epos_temp = spark.sql('select * from ivs_with_epos_temp')
-- MAGIC
-- MAGIC retailer_item_ids_temp = df_ivs_with_epos_temp.select("RefExternal").distinct()
-- MAGIC
-- MAGIC retailer_item_ids_temp.count()

-- COMMAND ----------

CREATE OR REPLACE TEMPORARY VIEW ivs_with_epos_temp2
AS SELECT 
          iv.CallfileId
          , iv.CallfileVisitId
          , iv.InterventionCount
...
-- MAGIC %python
-- MAGIC # RUN ME
-- MAGIC df_ivs_with_epos_temp2 = spark.sql('select * from ivs_with_epos_temp2')
-- MAGIC
-- MAGIC retailer_item_ids_temp2 = df_ivs_with_epos_temp.select("RefExternal").distinct()
-- MAGIC
-- MAGIC retailer_item_ids_temp2.count()
...
-- MAGIC %python
-- MAGIC # df_ivs_with_epos_debug = df_ivs_with_epos.select("RefExternal").where(pyf.col("RefExternal") == 1302949).distinct()
-- MAGIC
-- MAGIC # display(df_ivs_with_epos_debug)
...
-- MAGIC %python
-- MAGIC
-- MAGIC # retailer_item_ids.write.mode('overwrite').format('delta').save('/mnt/processed/temp/rbusahealth_walmart_inv_epos-retailer_item_ids')
