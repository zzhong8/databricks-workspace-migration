...
def function_save_raw_alerts_into_table(df):
    column_mapping = {
    'Lkp_productGroupId': 'LKP_PRODUCT_GROUP_ID',
    'RETAILER_ITEM_ID': 'RETAILER_ITEM_ID',
    # 'TEST_ORGANIZATION_UNIT_NUM': 'ORGANIZATION_UNIT_NUM',
    'LastDateIncluded': 'LAST_DATE_INCLUDED',
    'LAST_OBS': 'LAST_OBS',
    'LAST_RATE': 'LAST_RATE',
    'LAST_LOST_SALES': 'LAST_LOST_SALES',
    'N_DAYS_LAST_SALE': 'N_DAYS_LAST_SALE',
    'PROB': 'PROB',
    'ZSCORE': 'ZSCORE',
    'R2': 'R2',
    'RMSE': 'RMSE',
    ...
    # 'OutletId': 'ORGANIZATION_UNIT_NUM',
    'notes': 'NOTES',
    'ChainRefExternal': 'CHAIN_REF_EXTERNAL',
    'Inventory': 'INVENTORY',
    'PRICE': 'PRICE',
    'LostSalesValue': 'LOST_SALES_VALUE',
    'WeeksCover': 'WEEKS_COVER',
    # 'CapValue': 'CAP_VALUE',
    'TARSection': 'TAR_SECTION',
    'Issue': 'ISSUE',
    ...
    raw_alerts_schema = pyt.StructType([
        ...
        pyt.StructField('LKP_PRODUCT_GROUP_ID', pyt.IntegerType(), True),
        pyt.StructField('UNIVERSAL_PRODUCT_CODE', pyt.StringType(), True),
        pyt.StructField('CHAIN_REF_EXTERNAL', pyt.StringType(), True),
        pyt.StructField('ALERT_ID', pyt.StringType(), True),
        pyt.StructField('Id', pyt.IntegerType(), True),
        pyt.StructField('LAST_DATE_INCLUDED', pyt.DateType(), True),
        ...
        pyt.StructField('LOST_SALES_VALUE', pyt.FloatType(), True),
        ...
        pyt.StructField('TAR_SECTION', pyt.IntegerType(), True),
        pyt.StructField('PROMOTION_NAME', pyt.StringType(), True),
        pyt.StructField('WEEKS_COVER', pyt.IntegerType(), True),
        pyt.StructField('DAYS_SINCE_PROMO', pyt.StringType(), True),
        pyt.StructField('DATE_GENERATED', pyt.DateType(), True),
        pyt.StructField('ALERT_TYPE', pyt.StringType(), True),
    ])

    raw_alert_table_cols = [col.name for col in raw_alerts_schema]
    # Define the list of columns in the desired order based on your table schema
    desired_order = [
        'RETAILER',
        'CLIENT',
        'COUNTRY_CODE',
    ...
    df_raw_alerts_sorted = df_raw_alerts.select(desired_order)
    # df_raw_alerts_sorted.printSchema()

    # assert all(
    #     (a.name, a.dataType) == (b.name, b.dataType)
    #     for a, b in zip(df_raw_alerts_sorted.schema, raw_alerts_schema)
    #     ), 'Inconsistent Schema'

    df_raw_alerts_sorted.write.format("delta")\
        .mode('append')\
...
def function_split_run_test(df):
    ...
    if 'Store#' in df_test_stores.schema.names:
        df_test_stores = df_test_stores.withColumnRenamed('Store#', 'StoreID')
