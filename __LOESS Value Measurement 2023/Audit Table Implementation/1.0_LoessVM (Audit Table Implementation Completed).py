...
df_intervention_pre_0 = (
    df_intervention_pre.where(pyf.col("call_date") >= min_date_filter).dropDuplicates()
)

print(f"{df_intervention_pre_0.cache().count():,} Total distinct interventions in input view within the last 6 months (this is the starting point for our audit table)")
...
print(f"{df_intervention_1.cache().count():,} Candidate interventions to measure after removal of interventions with inferior rank")
...
print(f"{df_intervention_2.cache().count():,} Candidate interventions to measure after removing interventions with NULL values in the aggregation column (aka Interventions to measure")
...
if upc_rollup_flag == 1:
    print(f"{df_intervention_3.count():,} Interventions to measure that have matching ePOS data on the UPC")

else:
    print(f"{df_intervention_3.count():,} Interventions to measure that have matching ePOS data on the epos_retailer_item_id")
...
step_2 = df_intervention_pre_2.select(pyf.col("response_id").alias("has_valid_epos_retailer_item_id"))
step_3 = df_intervention_1.select(pyf.col("response_id").alias("has_rank_priority"))
step_4 = df_intervention_2.select(pyf.col("response_id").alias("has_valid_partition_col_value"))
step_5 = df_intervention_3.select(pyf.col("response_id").alias("has_matching_epos_data"))
step_6 = df_results.select(pyf.col("response_id").alias("measured_responses"))
step_7 = df_results_final.select(pyf.col("response_id").alias("completed_measured_responses"))
...
df_stats = df_intervention_pre_0.join(
    ...
    step_2, df_intervention_pre_0.response_id == step_2.has_valid_epos_retailer_item_id, 'left').join(
    step_3, df_intervention_pre_0.response_id == step_3.has_rank_priority, 'left').join(  
    step_4, df_intervention_pre_0.response_id == step_4.has_valid_partition_col_value, 'left').join(  
    step_5, df_intervention_pre_0.response_id == step_5.has_matching_epos_data, 'left').join(   
    step_6, df_intervention_pre_0.response_id == step_6.measured_responses, 'left').join( 
    step_7, df_intervention_pre_0.response_id == step_7.completed_measured_responses, 'left').sort(("response_id"))
...
for col in cols:
    df_stats = df_stats.withColumn(col, pyf.when(df_stats[col].isNull(), 0).otherwise(1))
...
mapping_expr = pyf.create_map([pyf.lit(x) for x in chain(*status_code_dictionary.items())])
