...
# # Temp - artificially provide a mapping file
# agg_col = "UPC"
# upc_rollup_flag = 1
...
# upc_itemid_mapper = upc_itemid_mapper.withColumn("UPC", pyf.col("new_item_code")).drop("new_item_code", "reported_upc")

# print(upc_itemid_mapper.count())
...
if upc_rollup_flag == 1:
    ...
    df_pos_pre = df_pos_pre.join(
        upc_itemid_mapper,
        (df_pos_pre["HUB_RETAILER_ITEM_HK"] == upc_itemid_mapper["HUB_RETAILER_ITEM_HK"]),
        how="inner",
    ).drop(df_pos_pre["HUB_RETAILER_ITEM_HK"])
...
# Load LOESS baseline quantities - Temp

if upc_rollup_flag == -11:
    # Placeholder query until I can confirm with Bruce's recent updates
    df_sql_query_loess_baseline = f"""
        SELECT
            HUB_ORGANIZATION_UNIT_HK,
...
df_intervention_all_days = df_intervention_all_days.withColumn("upc", pyf.col("upc").cast("long"))
