...
# Debug Sainsburys (7745) Red Bull (17687)

# epos_retailer_item_id 90448768
...
try:
    upc_itemid_mapper = spark.sql(
        ...
        join {config_dict["epos_datavault_db_nm"]}.vw_latest_sat_epos_summary t2
...
upc_itemid_mapper_debug1 = spark.sql(
    f"""
    select
        distinct RETAILER_ITEM_ID from
    {config_dict["alertgen_im_db_nm"]}.LOESS_FORECAST_BASELINE_UNIT_UPC t1
    """
)

display(upc_itemid_mapper_debug1)

# COMMAND ----------

upc_itemid_mapper_debug2 = spark.sql(
    f"""
    select
        distinct
        t2.retailer_item_id
    from {config_dict["epos_datavault_db_nm"]}.vw_latest_sat_epos_summary t2
    """
)

display(upc_itemid_mapper_debug2)
...
df_intervention_pre_distinct_retailer_item_ids = df_intervention_pre.select(pyf.col("epos_retailer_item_id")).distinct()

display(df_intervention_pre_distinct_retailer_item_ids)

# COMMAND ----------

# Debug
df_intervention_pre_2 = df_intervention_pre_2.filter(pyf.col("epos_retailer_item_id") == 90448768)
display(df_intervention_pre_2)
...
step_4 = df_intervention_2.select(
    pyf.col("response_id").alias("has_unique_response")
)
...
status_code_dictionary = {
    "0": "1 Non-measurable standard response text",
    "1": "2 Null epos_retailer_item_id",
    "2": "5 Lower rank priority",
    "3": "4 Duplicative response",
    "4": "3 No matching ePOS data",
    "5": "0 Not properly measured",
    "6": "6 Measurement period not completed",
    "7": "7 Fully measured",
}
...
df_stats.coalesce(1).write.format("csv").option("header", "true").mode(
    ...
    f"/mnt/processed/loess_measurement/audit_tables/{client_id}-{country_id}-{holding_id}-{banner_id}-intervention-audit-table-{today_date}"
)
...
df_interventions_responses = spark.sql(
    f"""
    SELECT 
        DISTINCT call_id,
        epos_retailer_item_id,
...
# Identify if there are multiple response_ids for the same call and item
# Need to merge with just measured interventions to check for duplication
df_agg = (
    df_interventions_responses.select("call_id", "epos_retailer_item_id", "response_id")
    .union(
        df_summary_table.select(
            "call_id", "epos_retailer_item_id", "response_id"
        ).distinct()
    )
    .groupby("call_id", "epos_retailer_item_id")
    .agg(pyf.countDistinct("response_id").alias("unique_response_count"))
)

# If duplicated, remove from pool
df_agg = df_agg.filter(pyf.col("unique_response_count") == 1).select(
    "call_id", "epos_retailer_item_id"
)
df_interventions_responses = (
    df_interventions_responses.join(
        df_agg,
        (df_interventions_responses["call_id"] == df_agg["call_id"])
        & (
            df_interventions_responses["epos_retailer_item_id"]
            == df_agg["epos_retailer_item_id"]
        ),
    )
    .drop(df_agg["call_id"])
    .drop(df_agg["epos_retailer_item_id"])
)

# Join on unmeasured interventions
df_summary_table = df_summary_table.join(
    df_interventions_responses.filter(pyf.col("is_complete") == "false")
    .select("response_id")
    .distinct(),
    (df_summary_table["response_id"] == df_interventions_responses["response_id"]),
).drop(df_interventions_responses["response_id"])
