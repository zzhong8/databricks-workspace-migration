...
if upc_rollup_flag == 1:
    df_sql_query_loess_baseline = f"""
          ...
          L.HUB_ORGANIZATION_UNIT_HK,
            ...
            L.SALES_DT
          from {config_dict["alertgen_im_db_nm"]}.LOESS_FORECAST_BASELINE_UNIT_UPC L
          LEFT JOIN (
              SELECT HUB_RETAILER_ITEM_HK, MAX(UPC) as UPC
              FROM {config_dict["alertgen_im_db_nm"]}.LOESS_FORECAST_BASELINE_UNIT_UPC
              WHERE UPC <> HUB_RETAILER_ITEM_HK
              GROUP BY HUB_RETAILER_ITEM_HK, UPC
...
df_intervention_duplicative = df_intervention_duplicative.withColumn(
    ...
    pyf.when(
        ...
        pyf.lit("6 Duplicative response")
    )
)
...
df_stats = df_stats.drop(*columns_to_drop).dropDuplicates().cache()
