...
# Temp
upc_itemid_mapper = spark.read.format('delta').option('inferSchema', 'true').load(f'/mnt/processed/loess_measurement/cache/{client_id}-{country_id}-{holding_id}-{banner_id}-upc-itemid-mapper')

upc_itemid_mapper = upc_itemid_mapper.withColumn("UPC", pyf.col("new_item_code"))
...
# Aggregating the POS data to the essential POS columns
...
if upc_rollup_flag == 1:
    # Placeholder query until I can confirm with Bruce's recent updates
    ...
    df_loess_baseline = (
        df_loess_baseline.join(
            temp_mapping,
            (
                df_loess_baseline["HUB_RETAILER_ITEM_HK"]
                == temp_mapping["HUB_RETAILER_ITEM_HK"]
            ),
            how="inner",
        )
        .drop(df_loess_baseline["HUB_RETAILER_ITEM_HK"])
        .drop(temp_mapping["HUB_RETAILER_ITEM_HK"])
        .dropDuplicates()
    )
else:
    df_sql_query_loess_baseline = f"""
...
df_intervention_pre_0 = (
    df_intervention_pre.where(pyf.col("call_date") >= min_date_filter)
)

print(f"{df_intervention_pre_0.cache().count():,} Total interventions in input view within the last 6 months")

# Filter on interventions which are actionable
df_intervention_pre_1 = (df_intervention_pre_0
    .filter(df_intervention_pre_0["actionable_flg"].isNotNull())
)

print(f"{df_intervention_pre_1.cache().count():,} Interventions with an actionable response text")

# Filter on interventions which have an identifiable intervention_id and retailer_item_id
df_intervention_pre_2 = (df_intervention_pre_1
    .filter(df_intervention_pre_1["response_id"].isNotNull())
...
if upc_rollup_flag == 1:
    ...
    df_intervention_pre = (
        ...
        .drop(df_intervention_pre_2["epos_retailer_item_id"])
...
# print(f"{df_intervention_counts.cache().count():,} Candidate interventions to measure after UPC aggregation (if applicable) and removal of interventions with inferior rank")
...
print(f"{df_intervention_to_measure.cache().count():,} Candidate interventions to measure after removal of interventions with inferior rank")

# NEW CODE TO JOIN BACK IN
df_intervention_1 = df_intervention_to_measure.join(
    df_intervention_counts, on=partition_cols, how="inner"
)
# NEW CODE BLOCK ENDS HERE

print(f"{df_intervention_1.cache().count():,} Candidate interventions to measure after removing interventions with NULL values in the aggregation column (aka Interventions to measure")
...
df_intervention_2 = df_merged.dropDuplicates(partition_cols)

if upc_rollup_flag == 1:
    print(f"{df_intervention_2.count():,} Interventions to measure that have matching ePOS data on the UPC")

else:
    print(f"{df_intervention_2.count():,} Interventions to measure that have matching ePOS data on the epos_retailer_item_id")


# COMMAND ----------

display(df_intervention_2)
...
df_intervention_3 = df_intervention_2.where(
    pyf.col("measure_end") <= max_sales_date_filter
)

print(f"{df_intervention_3.cache().count():,} Interventions to measure that have completed their measurement period")
...
# # Formatting PRICE to successfully overwrite
...
if client_id in client_list:
    df_results = (
        ...
        .orderBy(
            ["epos_organization_unit_num", groupby_item_col, "call_date", "response_id"],
            ascending=True,
        )
    )
    df_results = (
        ...
        .withColumn("uplift_value", pyf.col("AVG_DIFF_POS_ITEM_QTY") * pyf.col("AVG_PRICE"))
    ...
    df_results = (
        ...
        .withColumn("total_intervention_effect_adjusted", pyf.col("SUM_DIFF_POS_ITEM_QTY"))
        .withColumn(
...
if upc_rollup_flag == 1:
    print(f"{df_results.count():,} Interventions measured pre UPC roll-up")
...
if upc_rollup_flag == 1:
    ...
    print(f"{df_results.count():,} Interventions measured post UPC roll-up")
...
# Filter out interventions that have not completed yet based on the date of the latest available POS data
df_results_final = df_results.where(
    pyf.col("measure_end") <= max_sales_date_filter
)

print(f"{df_results_final.count():,} Interventions measured that have completed their measurement period")
...
step_1 = df_intervention_pre_1.select(pyf.col("response_id").alias("measurable_standard_response_text"))
step_2 = df_intervention_pre_2.select(pyf.col("response_id").alias("has_epos_retailer_item_id"))
step_3 = df_intervention_pre_3.select(pyf.col("response_id").alias("completed_measurement_period"))
step_4 = df_intervention_1.select(pyf.col("response_id").alias("has_rolled_up_upc_and_priority"))
step_5 = df_intervention_2.select(pyf.col("response_id").alias("has_matching_epos_data"))
step_6 = df_results.select(pyf.col("response_id").alias("measured_response"))

# Creating the tracking table for dropped interventions
df_stats = df_intervention_pre_0.join(
    step_1, df_intervention_pre_0.response_id == step_1.measurable_standard_response_text, 'left').join(
    step_2, df_intervention_pre_0.response_id == step_2.has_epos_retailer_item_id, 'left').join(
    step_3, df_intervention_pre_0.response_id == step_3.completed_measurement_period, 'left').join(  
    step_4, df_intervention_pre_0.response_id == step_4.has_rolled_up_upc_and_priority, 'left').join(  
    step_5, df_intervention_pre_0.response_id == step_5.has_matching_epos_data, 'left').join(   
    step_6, df_intervention_pre_0.response_id == step_6.measured_response, 'left').sort(("response_id"))

# # Accounting for unmeasurable responses
...
# # Choose the table
# table_name = "acosta_retail_analytics_im.ds_intervention_summary"
# table = spark.table(table_name)

# # Define the filter condition
# filter_condition = (table.mdm_banner_id == banner_id) &\
#     (table.mdm_client_id == client_id) &\
#     (table.mdm_country_id == country_id) &\
#     (table.mdm_holding_id == holding_id)

# # Update the table
# table = table.withColumn(
#     "is_complete", 
#     pyf.when(filter_condition, False).otherwise(table.is_complete)
# )
