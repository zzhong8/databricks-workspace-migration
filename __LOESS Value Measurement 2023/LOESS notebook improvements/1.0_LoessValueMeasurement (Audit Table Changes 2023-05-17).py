...
status_code_dictionary = {
    ...
    "1": "2 Null or invalid epos_retailer_item_id",
...
df_stats_pivot_table = df_stats.groupBy("status_code").agg(pyf.count("*").alias("count_of_status_code")).orderBy(["status_code"], ascending=True)
...
df_stats.coalesce(1).write.format("csv").option("header", "true").mode(
    ...
    f"/mnt/processed/loess_measurement/audit_tables/{today_date}/{client_nm}-{banner_nm}-Intervention-Audit-Table-{today_date}"
)
...
df_stats_pivot_table.coalesce(1).write.format("csv").option("header", "true").mode(
    ...
    f"/mnt/processed/loess_measurement/audit_tables/{today_date}/{client_nm}-{banner_nm}-Intervention-Audit-Table-Summary-{today_date}"
)
