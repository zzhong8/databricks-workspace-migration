...
# 1st run ~24 mins, 2nd run ~ 24 mins
# View might not be complete - KHZ@Tesco is more like 304 unique item combintations
# Eric mentioned that we might need to verify retailer-client combinations in this view
upc_id = spark.sql("select * from default.vw_upc_retailer_id")

store_ids = spark.sql(
    f"""select store_id, subbanner_description, division_description from acosta_retail_analytics_im.vw_dimension_store where holding_id = {holding_id}"""
)

upc_id_banner = (
    upc_id.join(store_ids, "store_id")
    .dropDuplicates(["retailer_item_id", "upc", "division_description"])
    .select("retailer_item_id", "upc", "division_description")
)
upc_id_banner = upc_id_banner.filter(
    pyf.col("division_description") == config_dict["mdm_banner_nm"]
)

grouped_df = upc_id_banner.groupBy("retailer_item_id").agg(
    pyf.collect_set("upc").alias("upc_set")
)
joined_df = (
    grouped_df.alias("a")
    .join(grouped_df.alias("b"), pyf.lit(1) == pyf.lit(1))
    .where(
        pyf.size(pyf.array_intersect(pyf.col("a.upc_set"), pyf.col("b.upc_set"))) >= 1
    )
    .select(
        pyf.col("a.retailer_item_id").alias("retailer_item_id"),
        pyf.array_union(pyf.col("a.upc_set"), pyf.col("b.upc_set")).alias("upc_set"),
    )
)
temp_df = (
    joined_df.groupBy("retailer_item_id")
    .agg(pyf.collect_set("upc_set").alias("upc_set"))
    .withColumn(
        "upc_set", pyf.sort_array(pyf.array_distinct(pyf.flatten(pyf.col("upc_set"))))
    )
)

temp_df = temp_df.withColumn("length", pyf.size(temp_df["upc_set"]))
...
temp_df = temp_df.select(
    ...
    pyf.explode(
        pyf.array(
            [
                pyf.array([pyf.lit(temp_df.upc_set[0]), pyf.col("upc_set")[i]])
...
# Temporary code to add a random column to the dataframe - this will be replaced by the new mapping column
upc_itemid_mapper = upc_itemid_mapper.withColumn(
    agg_col, pyf.floor(pyf.rand() * 10) + 1
)
...
upc_itemid_mapper.write.format('delta')\
    .mode('overwrite')\
    .option('mergeSchema', 'true')\
    .save(f'/mnt/processed/loess_measurement/cache/{client_id}-{country_id}-{holding_id}-{banner_id}-upc-itemid-mapper')
...
upc_itemid_mapper = upc_itemid_mapper.withColumn("UPC", pyf.col("new_item_code")).drop("HUB_RETAILER_ITEM_HK")
...
### Temp
# Create the mocked loess baseline, in the future Bruce will provide an actual source to pull from - this is just a placeholder
...
# 1st run ~11 mins, 2nd run ~ X mins
...
df_intervention.write.format("delta").mode("overwrite").option(
    ...
    f"/mnt/processed/loess_measurement/cache/{client_id}-{country_id}-{holding_id}-{banner_id}-intervention"
)
...
cat_features_list = [
    ...
    "mdm_banner_id",
    "mdm_client_id",
    "store_acosta_number" "epos_organization_unit_num",
    agg_col,
...
# MAGIC %md
# MAGIC ## Save Cached Data (need to troubleshoot)
...
df_results = (
    ...
    .orderBy(
        ["epos_organization_unit_num", agg_col, "call_date", "response_id"],
        ascending=True,
    )
)
...
# create table if not exists acosta_retail_analytics_im.ds_intervention_summary
# (
