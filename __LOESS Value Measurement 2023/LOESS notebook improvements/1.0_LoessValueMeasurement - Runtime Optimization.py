...
try:
    upc_itemid_mapper = spark.sql(
        ...
        where t1.UPC <> t1.HUB_RETAILER_ITEM_HK
        """
    )
    upc_rollup_flag = 1
    agg_col = "UPC"
...
df_intervention_all.where(
    pyf.col("call_date") >= min_date_filter
).dropDuplicates().createOrReplaceTempView("t")
...
df_intervention_1.createOrReplaceTempView("t")
...
# Query the intervention view
df_intervention_2 = spark.sql(query)
...
df_intervention_2.createOrReplaceTempView("t")
...
display(df_intervention_duplicative)

# COMMAND ----------

# # Get the set of response ids that already have a status code assigned
# responses_ids_with_status_codes_already_assigned = (
#     df_stats.select("response_id").distinct().toPandas()["response_id"]
# )
# responses_ids_with_status_codes_already_assigned = set(responses_ids_with_status_codes_already_assigned)

# # Filter out response ids that already have a status code assigned; we don't want to double count in the audit table
# df_intervention_duplicative = df_intervention_duplicative.filter(
#     ~df_intervention_duplicative["response_id"].isin(responses_ids_with_status_codes_already_assigned)
# )

print(df_intervention_duplicative.count())

df_intervention_duplicative = df_intervention_duplicative.withColumn(
    "status_code", 
    pyf.when(
        df_intervention_duplicative["actionable_flg"].isNull(), 
        pyf.lit("1 Non-measurable standard response text")
    ).when(
        df_intervention_duplicative["response_id"].isNull(), 
        pyf.lit("2 Null or invalid epos_retailer_item_id")
    ).when(
        df_intervention_duplicative["epos_retailer_item_id"].isNull(), 
        pyf.lit("2 Null or invalid epos_retailer_item_id")
    ).otherwise(
        pyf.lit("4 Duplicative response")
    )
)

# df_intervention_duplicative.show()
df_stats = df_stats.union(df_intervention_duplicative).cache()
