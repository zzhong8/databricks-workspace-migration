...
df_intervention = spark.sql(f'''
    ...
    mdm_client_id = 16319 and
    mdm_holding_id = 3257 and
    coalesce(mdm_banner_id, -1) = 7746 and
    call_date = '2023-03-10' and
    store_acosta_number = '1794409'
''')
...
# ### Load from Cache ###
# df_intervention = spark.read.format('delta').load(
...
#     f'/mnt/processed/loess_measurement/results/{client_id}-{country_id}-{holding_id}-{banner_id}')
...
# # Overwriting production code for a micro sample (18 rows) of data - 2023-04-14
# # Took 55 seconds
...
# filter_condition = (table.mdm_client_id == client_id) & (table.call_id == "63156340")
...
# # Overwriting production code KHZ-Tesco
...
# # Query back the recently written data
# # Took 4.93 seconds and ran successfully to return 18 rows
# query = f"""
#     select * 
#     from acosta_retail_analytics_im.ds_intervention_summary source
#     where source.mdm_banner_id = {banner_id} and
#         source.mdm_client_id = {client_id} and
#         source.mdm_country_id = {country_id} and
#         source.mdm_holding_id = {holding_id} and
#         source.response_id in {tuple(df_summary_table.select('response_id').distinct().toPandas()['response_id'].unique())}
# """
