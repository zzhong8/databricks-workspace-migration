...
def build_pos_table(retailer, client, country_code, start_date, end_date):
    ...
    print('Build table for {}_{}_{} from {}'.format(retailer, client, country_code, start_date))
...
for retailer, client, country_code in input_data_list:
    ...
    if start_date == None:
        start_date = startDate_df[((startDate_df.RETAILER == retailer) & (startDate_df.CLIENT == client)) \
            & (startDate_df.COUNTRY_CODE == country_code)]['MAX_DATE'].values.astype('str')[0]
...
# MAGIC               )
# MAGIC   VALUES (RETAILER_ITEM_ID, RETAILER_ITEM_DESC, RETAILER, CLIENT,
# MAGIC               COUNTRY_CODE, ORGANIZATION_UNIT_NUM, SALES_DT, POS_ITEM_QTY,
# MAGIC               POS_AMT, ON_HAND_INVENTORY_QTY, DRFE_POS_ITEM_QTY,
# MAGIC               DRFE_ERROR, DRFE_LOAD_TS, MODEL_TYPE,
# MAGIC               TEST_SET_MSE_PERFORMANCE, High_Dollar_Sales,
# MAGIC               High_Frequency, Low_Frequency
# MAGIC               )
