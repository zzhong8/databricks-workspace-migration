...
# MAGIC %md
# MAGIC ## Exercise 1:
# MAGIC We are looking to get a list of the top selling items that fall between 25% of the median price. Using Koalas please do the following
...
koalas_retail[(koalas_retail['UnitPrice'] > 1.25) & (koalas_retail['UnitPrice'] < 4.13)].sort_values(by='Quantity', ascending = False).head()
...
# MAGIC %md
# MAGIC ## Exercise 2:
# MAGIC Let's build a plot that uses the retail data from **Exercise 1** to look at the top 20 selling products. Please use Koalas to do the following:
# MAGIC - Group by the item
# MAGIC - Select the 20 items that have sold the most quantity 
...
# MAGIC %md
# MAGIC You'll notice that the equivalent in Koalas is not possible because it requires collecting all the data back to the driver.
...
# MAGIC %md
# MAGIC In order to work around this, you'll have to convert your Koalas DF to a Pandas on and call `to_list()` from there

# COMMAND ----------

# DBTITLE 1,Converting to an Index/Column to a List in Koalas
