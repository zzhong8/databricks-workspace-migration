...
# MAGIC %md
# MAGIC # Date/Country Dimension
# MAGIC
# MAGIC This notebook creates a date/country dimension table for the DRFE system.  Note that it both *creates* (i.e. DDL) the table **and** *populates* the table. The result is a table with a row for each date/country code combination.
# MAGIC (e.g. `11/16/2018 | US` and `11/16/2018 | UK` are separate rows in the table)
# MAGIC
# MAGIC Most of the logic is concerned with labeling days as they are related to their country-specific holidays.  This includes identifying the days on which holidays fall for each country
# MAGIC AND identifying the days that lead up to and follow the holidays.  We do not want to be responsible for maintaining the logic on what days the holidays fall so we rely on the
# MAGIC `holidays` package to do that for us.
# MAGIC
# MAGIC The output dataframe uses a wide format (approximately 400 columns).  This allows the categorical data (i.e. holidays and their leading and lagging days) to be encoded for use
# MAGIC with machine learning modeling.
...
# Expand the specified range of years by one year in each direction.  This will let us look up holidays outside of the specified range
...
for countryCode in countryCodes:
  ...
  for y in years:
    ...
    row_list.append((countryCode, easter.easter(y), "Easter"))            # Easter logic is very complex; use the "easter" library to determine the date of Easter Sunday
    
    if countryCode == "US":
      row_list.append((countryCode, date(y, 2, 1) + relativedelta(weekday=SU(+1)), "Super Bowl"))   # Super Bowl is currently played on the first Sunday in February.  That could change in the future
...
# Get the holidays that have the same name but fall on different days in different countries (e.g. US and Canada both have "Thanksgiving" but it's on different days)
names = holidays_df.groupby("name", "year").agg(pyf.countDistinct("date").alias("date_count")).filter("date_count > 1").select("name").distinct().rdd.flatMap(lambda x: x).collect()
...
# Now we need to compute the "relative" column values.  This is a continuous feature that indicates how many days since/until the closest occurrence of a holiday.
# For example:  When SALES_DT is 12/20/2018, then "CHRISTMAS.DAY_RELATIVE" should be 5... because it's 5 days until the next occurrence of Christmas.
#               When SALES_DT is 12/28/2018, then "CHRISTMAS.DAY_RELATIVE" should be -3... because the most recent occurrence of Christmas was three days in the past
...
for x in holidayNames:
  ...
  # For each day, join it to all past and future occurrences of the current holiday.  This creates an exploded rowset (i.e. many more rows than in the original "days" dataset)
  z = days.alias("d")\
        .join(holidays_df.filter(holidays_df.name == x).alias("p"), (pyf.col("p.country") == pyf.col("d.COUNTRY")) & (pyf.col("p.date") <  pyf.col("d.SALES_DT")), "outer")\
        .join(holidays_df.filter(holidays_df.name == x).alias("n"), (pyf.col("n.country") == pyf.col("d.COUNTRY")) & (pyf.col("n.date") >= pyf.col("d.SALES_DT")), "outer")
  ...
  # All of this repeated joining causes our distributed data set to get scattered.  This causes the time required for each iteration of the loop to double every time.
...
for i in hw.collect():
  ...
  for j in range(start, end + 1):
    ...
    days = days.withColumn(columnName, pyf.when(days[relativeColumn] == j * -1, 1).otherwise(0).cast(ShortType())).orderBy("COUNTRY", "SALES_DT")
