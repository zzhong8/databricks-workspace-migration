...
# If client is a UK client, then they need auditing functionality
# if (country_id == 30):
...
#     dbutils.notebook.run("../../dataplatform-retail-forecastengine/Measurement/3.0_PostHocCorrections (Write to Temp Table)", 0, {"country_id": country_id, "client_id": client_id, "holding_id": holding_id, "banner_id": banner_id})

# else:
dbutils.notebook.run("./1.0_CacheMeasurementData (Read from Temp Table)", 0, {"country_id": country_id, "client_id": client_id, "holding_id": holding_id, "banner_id": banner_id})

dbutils.notebook.run("./2.0_Fitting", 0, {"country_id": country_id, "client_id": client_id, "holding_id": holding_id, "banner_id": banner_id})

dbutils.notebook.run("./3.0_PostHocCorrections (Write to Temp Table)", 0, {"country_id": country_id, "client_id": client_id, "holding_id": holding_id, "banner_id": banner_id})
