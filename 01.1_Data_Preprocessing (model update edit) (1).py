...
if not is_nowcast: 
    ...
    database = f'{SOURCE_SYSTEM}_{RETAILER}_{CLIENT}_{COUNTRY_CODE}_dv'
    print('Accessing', database)

    store_names_query = '''
    SELECT SOU.ORGANIZATION_UNIT_NUM
       FROM {database}.vw_latest_sat_organization_unit SOU 
    '''
...
if not is_nowcast: 
    data = data.join(champions, on="RETAILER_ITEM_ID", how="left_anti")
...
if not is_nowcast: 
    # This filter requires at least 42 days of non-zero sales in the entire dataset
...
# #temporary to make the development notebook work - do we actually need pygam tho?
# !pip install pygam
