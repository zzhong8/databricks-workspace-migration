...
sql_all_measurable_sainsburys_nestle_uk_rep_responses_and_results_202210 = """
SELECT distinct
a.*,
c.RETAILER_ITEM_ID,
e.SALES_DT,
...
left join (select * from sainsburys_nestlecore_uk_dv.vw_sat_link_epos_summary) e on
...
and
a.mdm_banner_id = 7745 -- Sainsburys
and
...
df_all_measurable_sainsburys_nestle_uk_rep_responses_and_results_202210 = spark.sql(sql_all_measurable_sainsburys_nestle_uk_rep_responses_and_results_202210).cache()

df_all_measurable_sainsburys_nestle_uk_rep_responses_and_results_202210.coalesce(1).write.format('csv')\
    .option('header', 'true')\
    .mode('overwrite')\
    .save(f'/mnt/artifacts/hugh/all_measurable_sainsburys_nestle_uk_rep_responses_and_results_oct_2022_20230302')
