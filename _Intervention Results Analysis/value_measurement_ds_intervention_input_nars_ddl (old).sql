...
create or replace view acosta_retail_report_im.vw_ds_intervention_input_dla as
...
from
  mdm.vw_lookup_stores s
  left outer join mdm.hs_division as d on s.division_id = d.divisionid
  left outer join mdm.hs_holding as h on d.holdingid = h.holdingid
...
(
  ...
  from 
    acosta_retail_report_im.interventions_retailer_client_config c,
    acosta_retail_report_im.interventions_response_mapping map,
    acosta_retail_report_im.interventions_parameters param
  where
...
create or replace view acosta_retail_report_im.vw_ds_intervention_input_uk_opportunities as
-- This sub query gets the retailer customer store(Division Number - Store Number)
-- from Global Connect/BOBv2 along with MDM Acosta Store Number
with gc_kroger_stores as
(
...
(
  ...
  where
    ...
    map.objective_typ = 'UK Opportunities'
),
objective_tag as
(
  select
    tag.name as objective_typ,
    objtag.obj_id as question_id
  from
    nars.tag,
    nars.objtag
  where
    tag.batch_ts >= 20200701000000 and
    objtag.batch_ts >= 20200701000000 and
    tag.name in ('UK Opportunities') and
    tag.tag_id = objtag.tag_id
  group by
    objective_typ,
    question_id
)
...
from
  nars.aiau
  inner join objective_tag tags
  on
    aiau.question_id = tags.question_id and
    tags.objective_typ = 'UK Opportunities'
  inner join mdm_store
  on
...
create or replace view acosta_retail_report_im.vw_ds_intervention_input_nars as
select 
mdm_country_id,
mdm_country_nm,
mdm_holding_id,
...
actionable_flg
 from acosta_retail_report_im.vw_ds_intervention_input_dla
...
intervention_end_day,
actionable_flg
 from acosta_retail_report_im.vw_ds_intervention_input_uk_opportunities
