...
@pyf.pandas_udf('int')
def hash_org_unit_num(org_unit_num_string: pd.Series) -> pd.Series:
    return org_unit_num_string.apply(hash_func)


if 'string' in dict(output_data.dtypes).get('ORGANIZATION_UNIT_NUM', ''):
    output_data = output_data.withColumn(
        'ORGANIZATION_UNIT_NUM',
        hash_org_unit_num(pyf.col('ORGANIZATION_UNIT_NUM'))
    )
