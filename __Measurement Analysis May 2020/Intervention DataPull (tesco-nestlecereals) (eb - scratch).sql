...
select * from bobv2.Company where FullName like 'NEST%'
...
select * from bobv2.Campaign where CompanyID = '446'
...
select * from bobv2.callfile where campaignid = '3426'

-- COMMAND ----------

select * from bobv2.callfile where campaignid = '4028'

-- COMMAND ----------

select * from bobv2.callfile where CampaignId = '3014'
...
-- MAGIC %python
-- MAGIC # # RUN ME
-- MAGIC # df_iv = spark.sql('select * from adv_analytics_IV where ParentChainName = \'Tesco - Parent\'')
...
-- MAGIC
-- MAGIC # df_iv.write.mode('overwrite').format('delta').save('/mnt/processed/temp/intervention_data-nestlecereals-tesco')
...
-- MAGIC %python
-- MAGIC # RUN ME
-- MAGIC df_iv1 = spark.sql('select * from adv_analytics_IV where ParentChainName = \'Morrisons - Parent\'')
...
-- MAGIC # print(df_iv.count())
-- MAGIC
-- MAGIC df_iv1.write.mode('overwrite').format('delta').save('/mnt/processed/temp/intervention_data-nestlecereals-morrisons')

-- COMMAND ----------

-- MAGIC %python
-- MAGIC # RUN ME
-- MAGIC df_iv2 = spark.sql('select * from adv_analytics_IV where ParentChainName = \'Sainsburys - Parent\'')
...
-- MAGIC # print(df_iv.count())
-- MAGIC
-- MAGIC df_iv2.write.mode('overwrite').format('delta').save('/mnt/processed/temp/intervention_data-nestlecereals-sainsburys')

-- COMMAND ----------

-- MAGIC %python
-- MAGIC # RUN ME
-- MAGIC df_iv3 = spark.sql('select * from adv_analytics_IV where ParentChainName = \'Asda - Parent\'')
...
-- MAGIC # print(df_iv.count())
-- MAGIC
-- MAGIC df_iv3.write.mode('overwrite').format('delta').save('/mnt/processed/temp/intervention_data-nestlecereals-asda')
...
-- MAGIC
-- MAGIC sql = "SELECT \
-- MAGIC           \"tesco_nestlecereals_uk\" AS RETAIL_CLIENT \
-- MAGIC           , 16 AS ParentChainId \
-- MAGIC           , s.SALES_DT \
...
-- MAGIC         FROM tesco_nestlecereals_uk_dv.vw_sat_link_epos_summary s \
-- MAGIC         INNER JOIN tesco_nestlecereals_uk_dv.hub_retailer_item ri \
-- MAGIC           ON s.HUB_RETAILER_ITEM_HK = ri.HUB_RETAILER_ITEM_HK \
-- MAGIC         INNER JOIN tesco_nestlecereals_uk_dv.hub_organization_unit ou \
-- MAGIC           ON s.HUB_ORGANIZATION_UNIT_HK = ou.HUB_ORGANIZATION_UNIT_HK \
-- MAGIC         LEFT JOIN tesco_nestlecereals_uk_retail_alert_im.vw_drfe_forecast_baseline_unit d \
...
-- MAGIC
-- MAGIC sql1 = "SELECT \
-- MAGIC           \"morrisons_nestlecereals_uk\" AS RETAIL_CLIENT \
-- MAGIC           , 24 AS ParentChainId \
-- MAGIC           , s.SALES_DT \
-- MAGIC           , s.HUB_ORGANIZATION_UNIT_HK \
-- MAGIC           , s.HUB_RETAILER_ITEM_HK \
-- MAGIC           , s.POS_ITEM_QTY \
...
-- MAGIC           , ou.ORGANIZATION_UNIT_NUM \
-- MAGIC           , d.BASELINE_POS_ITEM_QTY \
-- MAGIC         FROM morrisons_nestlecereals_uk_dv.vw_sat_link_epos_summary s \
-- MAGIC         INNER JOIN morrisons_nestlecereals_uk_dv.hub_retailer_item ri \
-- MAGIC           ON s.HUB_RETAILER_ITEM_HK = ri.HUB_RETAILER_ITEM_HK \
-- MAGIC         INNER JOIN morrisons_nestlecereals_uk_dv.hub_organization_unit ou \
-- MAGIC           ON s.HUB_ORGANIZATION_UNIT_HK = ou.HUB_ORGANIZATION_UNIT_HK \
-- MAGIC         LEFT JOIN morrisons_nestlecereals_uk_retail_alert_im.vw_drfe_forecast_baseline_unit d \
-- MAGIC         /* This could be replaced with expected baseline table */ \
-- MAGIC           ON s.SALES_DT = d.SALES_DT \
-- MAGIC           AND s.HUB_ORGANIZATION_UNIT_HK = d.HUB_ORGANIZATION_UNIT_HK \
-- MAGIC           AND s.HUB_RETAILER_ITEM_HK = d.HUB_RETAILER_ITEM_HK"
-- MAGIC
-- MAGIC dfEPOS1 = spark.sql(sql1)
-- MAGIC dfEPOS1.cache().createOrReplaceTempView("adv_analytics_IV_epos_sales1")

-- COMMAND ----------

select * from adv_analytics_IV_epos_sales1 limit 10

-- COMMAND ----------

CREATE OR REPLACE TEMPORARY VIEW ivs_with_epos1
AS SELECT 
          iv.CallfileId
          , iv.CallfileVisitId
        ...
        LEFT JOIN adv_analytics_IV_epos_sales1 s
...
select * from ivs_with_epos1 limit 50

-- COMMAND ----------

-- MAGIC %python
-- MAGIC # RUN ME
-- MAGIC df_ivs_with_epos1 = spark.sql('select * from ivs_with_epos1')
-- MAGIC
-- MAGIC df_ivs_with_epos1.write.mode('overwrite').format('delta').save('/mnt/processed/temp/nestlecereals_morrisons_inv_epos')
...
-- MAGIC
-- MAGIC sql2 = "SELECT \
-- MAGIC           \"sainsburys_nestlecereals_uk\" AS RETAIL_CLIENT \
-- MAGIC           , 199 AS ParentChainId \
-- MAGIC           , s.SALES_DT \
-- MAGIC           , s.HUB_ORGANIZATION_UNIT_HK \
-- MAGIC           , s.HUB_RETAILER_ITEM_HK \
-- MAGIC           , s.POS_ITEM_QTY \
...
-- MAGIC           , ou.ORGANIZATION_UNIT_NUM \
-- MAGIC           , d.BASELINE_POS_ITEM_QTY \
-- MAGIC         FROM sainsburys_nestlecereals_uk_dv.vw_sat_link_epos_summary s \
-- MAGIC         INNER JOIN sainsburys_nestlecereals_uk_dv.hub_retailer_item ri \
-- MAGIC           ON s.HUB_RETAILER_ITEM_HK = ri.HUB_RETAILER_ITEM_HK \
-- MAGIC         INNER JOIN sainsburys_nestlecereals_uk_dv.hub_organization_unit ou \
-- MAGIC           ON s.HUB_ORGANIZATION_UNIT_HK = ou.HUB_ORGANIZATION_UNIT_HK \
-- MAGIC         LEFT JOIN sainsburys_nestlecereals_uk_retail_alert_im.vw_drfe_forecast_baseline_unit d \
-- MAGIC         /* This could be replaced with expected baseline table */ \
-- MAGIC           ON s.SALES_DT = d.SALES_DT \
-- MAGIC           AND s.HUB_ORGANIZATION_UNIT_HK = d.HUB_ORGANIZATION_UNIT_HK \
-- MAGIC           AND s.HUB_RETAILER_ITEM_HK = d.HUB_RETAILER_ITEM_HK"
-- MAGIC
-- MAGIC dfEPOS2 = spark.sql(sql2)
-- MAGIC dfEPOS2.cache().createOrReplaceTempView("adv_analytics_IV_epos_sales2")

-- COMMAND ----------

select * from adv_analytics_IV_epos_sales2 limit 10

-- COMMAND ----------

CREATE OR REPLACE TEMPORARY VIEW ivs_with_epos2
AS SELECT 
          iv.CallfileId
          , iv.CallfileVisitId
          , iv.InterventionCount
          ...
          , s.BASELINE_POS_ITEM_QTY
        FROM adv_analytics_IV iv
        LEFT JOIN adv_analytics_IV_epos_sales2 s
          ON iv.ChainRefExternal = s.ORGANIZATION_UNIT_NUM
          AND iv.RefExternal = s.RETAILER_ITEM_ID
        WHERE
          (DATE_ADD(CAST(iv.InterventionDate AS DATE), DayStart) <= s.SALES_DT
...
select * from ivs_with_epos2 limit 50

-- COMMAND ----------

-- MAGIC %python
-- MAGIC # RUN ME
-- MAGIC df_ivs_with_epos2 = spark.sql('select * from ivs_with_epos2')
-- MAGIC
-- MAGIC df_ivs_with_epos2.write.mode('overwrite').format('delta').save('/mnt/processed/temp/nestlecereals_sainsburys_inv_epos')
...
-- MAGIC
-- MAGIC sql3 = "SELECT \
-- MAGIC           \"asda_nestlecereals_uk\" AS RETAIL_CLIENT \
-- MAGIC           , 23 AS ParentChainId \
-- MAGIC           , s.SALES_DT \
-- MAGIC           , s.HUB_ORGANIZATION_UNIT_HK \
-- MAGIC           , s.HUB_RETAILER_ITEM_HK \
-- MAGIC           , s.POS_ITEM_QTY \
...
-- MAGIC           , ou.ORGANIZATION_UNIT_NUM \
-- MAGIC           , d.BASELINE_POS_ITEM_QTY \
-- MAGIC         FROM asda_nestlecereals_uk_dv.vw_sat_link_epos_summary s \
-- MAGIC         INNER JOIN asda_nestlecereals_uk_dv.hub_retailer_item ri \
-- MAGIC           ON s.HUB_RETAILER_ITEM_HK = ri.HUB_RETAILER_ITEM_HK \
-- MAGIC         INNER JOIN asda_nestlecereals_uk_dv.hub_organization_unit ou \
-- MAGIC           ON s.HUB_ORGANIZATION_UNIT_HK = ou.HUB_ORGANIZATION_UNIT_HK \
-- MAGIC         LEFT JOIN asda_nestlecereals_uk_retail_alert_im.vw_drfe_forecast_baseline_unit d \
-- MAGIC         /* This could be replaced with expected baseline table */ \
-- MAGIC           ON s.SALES_DT = d.SALES_DT \
-- MAGIC           AND s.HUB_ORGANIZATION_UNIT_HK = d.HUB_ORGANIZATION_UNIT_HK \
-- MAGIC           AND s.HUB_RETAILER_ITEM_HK = d.HUB_RETAILER_ITEM_HK"
-- MAGIC
-- MAGIC dfEPOS3 = spark.sql(sql3)
-- MAGIC dfEPOS3.cache().createOrReplaceTempView("adv_analytics_IV_epos_sales3")

-- COMMAND ----------

select * from adv_analytics_IV_epos_sales3 limit 10

-- COMMAND ----------

CREATE OR REPLACE TEMPORARY VIEW ivs_with_epos3
AS SELECT 
          iv.CallfileId
          , iv.CallfileVisitId
          , iv.InterventionCount
          ...
          , s.BASELINE_POS_ITEM_QTY
        FROM adv_analytics_IV iv
        LEFT JOIN adv_analytics_IV_epos_sales3 s
          ON iv.ChainRefExternal = s.ORGANIZATION_UNIT_NUM
          AND iv.RefExternal = s.RETAILER_ITEM_ID
        WHERE
          (DATE_ADD(CAST(iv.InterventionDate AS DATE), DayStart) <= s.SALES_DT
...
select * from ivs_with_epos3 limit 50

-- COMMAND ----------

-- MAGIC %python
-- MAGIC # RUN ME
-- MAGIC df_ivs_with_epos3 = spark.sql('select * from ivs_with_epos3')
-- MAGIC
-- MAGIC df_ivs_with_epos3.write.mode('overwrite').format('delta').save('/mnt/processed/temp/nestlecereals_asda_inv_epos')
...
-- MAGIC
-- MAGIC df_ivs_with_epos.write.mode('overwrite').format('delta').save('/mnt/processed/temp/nestlecereals_tesco_inv_epos')
